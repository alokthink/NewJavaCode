...
public class DocumentReferenceDaoImpl implements DocumentReferenceDao{
	...
	public List<Object> getDocumentReferenceList(String type,String limit) throws SQLException {
		...
		try {
			String query="";
			if("all".equals(type))
				query = "SELECT DISTINCT TOP "+limit+" view_document_reference_batch.* FROM view_document_reference_batch WHERE type IN (0, 1, 2, 4) ORDER BY document_reference_id DESC";
			else
				query="SELECT DISTINCT TOP 100 view_document_reference_batch.* FROM view_document_reference_batch WHERE type in (0,2) and active = 1 ORDER BY document_reference_id DESC";
	...
	public List<Object> getSearchDocumentReferenceList(DocumentReferenceAttributeModel documentReferenceAttributeModel) throws SQLException {
		...
		try {
			...
			StringBuilder query = new StringBuilder("SELECT DISTINCT TOP " + documentReferenceAttributeModel.getLimit()+
					" view_document_reference_batch.* FROM view_document_reference_batch where type IN (");
