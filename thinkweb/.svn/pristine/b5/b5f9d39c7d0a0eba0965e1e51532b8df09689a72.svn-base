...
public class PaymentInfoDaoImpl implements PaymentInfoDao {
	...
	public List<DropdownModel> getPaymentType() {
		...
		try {
			rows = jdbcTemplate
					.queryForList("select payment_type,description,credit_card_type,payment_form from payment_type;");
			for (Map<String, Object> row : rows) {
				...
				model.setExtraDataDef(
						row.get("credit_card_type") != null ? row.get("credit_card_type").toString() : "");
	...
	public PaymentModel getCustomerDetailsForPayment(String customerId) {
		...
		try {
			customerDetailsList = jdbcTemplate.queryForList(
					"select CONCAT(c.fname,' ' collate Latin1_General_CI_AS,c.lname)as nameOnCard,CONCAT(c.fname,' ' collate Latin1_General_CI_AS,c.lname)as fullname,c.customer_id,c.creation_date,ca.address1,ca.city from customer c inner join customer_address ca on c.customer_id =ca.customer_id where  c.customer_id="
							+ customerId);
	...
	public List<DropdownModel> getPayerCustomer(int customerId, int orderId) {
		...
		try {
			...
			if (serviceSeqresult != null) {
				if (orderId != 0) {
					rows = jdbcTemplate.queryForList(
							"select c.customer_id,(isnull(c.fname,'')  + ' ' + isnull(c.lname,'')) as fullname,c.company from customer c "
				...
				rows = jdbcTemplate
						.queryForList("select c.customer_id,(isnull(c.fname,'')  + ' ' + isnull(c.lname,'')) as fullname from customer c "
...
//				String.class);
			...
			// Added by Malini to update mru_payment_seq in customer table 19/03/2020
			Map<String, Object> parametersToUpdate = new HashMap<String, Object>();
			String updateCustomerPaymentSeqQuery = "update customer set mru_payment_seq=:mru_payment_seq  where customer_id=:customer_id";
			String PaymentSeqValue = jdbcTemplate.queryForObject(
					"select max(payment_seq) from payment where customer_id =" + paymentModel.getCustomer_id(),
					String.class);
			if (PaymentSeqValue != null) {
				parametersToUpdate.put("mru_payment_seq", Integer.parseInt(PaymentSeqValue) + 1);
			} else {
				parametersToUpdate.put("mru_payment_seq", 1);
			}

			parametersToUpdate.put("customer_id", paymentModel.getCustomer_id());
			namedParameterJdbcTemplate.update(updateCustomerPaymentSeqQuery, parametersToUpdate);
		} catch (Exception e) {
			LOGGER.error(ERROR + e);
		}
	}
	 ...
	 * journal_id = jdbcTemplate.queryForObject("select id from mru_journal_id;"
		...
		try {
			...
			if (orderId == 0 && orderItemSeq == 0) {
				...
				for (CustomerOrderPayment row : rows) {
					...
					row.setDue(Double.valueOf((row.getGross_local_amount() - newpAmt)));
					row.setDue(Double.valueOf(df2.format(row.getDue())));
					if (row.getDue() == 0) {
						row.setDue(0.00);
					}
...
//							 }	 
				...
				fetchOrderDetails.append(
					...
					if (customerId != String.valueOf(bill_to_customer_id)) {
						fetchOrderDetails.append(
								"  and orderhdr_id =" + orderId + "and oi.pkg_item_seq is null or bill_to_customer_id = " + bill_to_customer_id);
					}
					fetchOrderDetails.append(
			...
			for (CustomerOrderPayment row : rows) {
				if (row.getGross_local_amount() == 0 || row.getGross_local_amount() == 0.00) {
					rows.remove(row);
				}
			}
...
//        parameters.put("suspend_after_n_settle_retries", paymentModel.getSuspendAfterNSettleRetries());
	...
	public List<Map<String, Object>> getDepositSummary(int customerId) {
		...
		try {
			depositSummary = jdbcTemplate.queryForList(
					"select currency,pay_exchange_rate, sum(customer_deposit_pay_amt) as customer_deposit_pay_amt,sum((pay_exchange_rate*customer_deposit_pay_amt)) as "
							+ "customer_deposit_base_amt from payment  where customer_id=" + customerId
							+ "and customer_deposit_pay_amt !=0 group by currency,pay_exchange_rate");

			/*
			 * depositSummary = jdbcTemplate.
			 * queryForList("select currency, sum(customer_deposit_pay_amt) as customer_deposit_pay_amt,sum((pay_exchange_rate*customer_deposit_pay_amt)) as customer_deposit_base_amt from payment  where  customer_id="
			 * + customerId + " and customer_deposit_pay_amt !=0 group by currency");
			 */
	...
	public List<CustomerOrderPayment> getOrderItemsDetailsEditPage(int customerId, int paymentSeq) {
		List<CustomerOrderPayment> orderItemDetailsforEditPayment = new ArrayList<>();
		Map<String, Object> dueAndPayDetails = new HashMap<>();
		List<Map<String, Object>> dueAndPay = new ArrayList<>();
		List<Map<String, Object>> paidOrderList = new ArrayList<>();

		try {
			double paidAmount = 0.0;
			double baseAmount = 0.0;
			double dueAmt = 0.0;
			paidOrderList = jdbcTemplate
					.queryForList("select orderhdr_id,order_item_seq,sum(pay_currency_amount)pay_currency_amount "
							+ "from paybreak where customer_id =" + customerId + " and payment_seq =" + paymentSeq
							+ " group by orderhdr_id,order_item_seq");
//			 
...
//			 dueAmt=baseAmount-paidAmount;
			...
			orderItemDetailsforEditPayment = jdbcTemplate.query(
					"select oc.order_code as order_code_id,oi.order_date,oi.refund_status,oi.gross_local_amount,oi.gross_local_amount as due,oi.payment_status,oi.order_status,oi.net_local_amount ,"
							+ "oi.order_code_id as order_code,oi.bill_to_customer_id,oi.customer_id,oi.currency,oi.order_qty ,oi.orderhdr_id,oi.gross_base_amount ,"
							+ "oi.net_base_amount,oi.order_item_seq  from order_item as oi inner join order_code as oc on  oi.order_code_id = oc.order_code_id "
							+ " where orderhdr_id in (select DISTINCT orderhdr_id from  order_item where orderhdr_id in"
			...
			for (CustomerOrderPayment orderItemDetailsforEditPayments : orderItemDetailsforEditPayment) {

				for (Map<String, Object> paidOrderLists : paidOrderList) {
					if (orderItemDetailsforEditPayments.getOrderhdr_id() == Integer
							.parseInt(paidOrderLists.get("orderhdr_id").toString())
							&& orderItemDetailsforEditPayments.getOrder_item_seq() == Integer
									.parseInt(paidOrderLists.get("order_item_seq").toString())) {
//						orderItemDetailsforEditPayments.setDueCurr();
						orderItemDetailsforEditPayments
								.setPayCurr(Double.parseDouble(paidOrderLists.get("pay_currency_amount").toString()));
					}
				}
...
//							+ customerId + "and payment_seq =" + paymentSeq + "))");
	...
	public List<Map<String, Object>> getDepositDetails(int customerId, String currency) {
		LOGGER.info("Inside deposit details");
		List<Map<String, Object>> depositDetails = new ArrayList<>();
		try {

			if (currency != null && currency != "" && !currency.toUpperCase().equals("[ALL]")) {

				depositDetails = jdbcTemplate.queryForList("select creation_date,(CASE when payment_clear_status=0 then 'Ready to Authorize' when payment_clear_status=1 then 'Authorized' when payment_clear_status=2 then 'Ready to Post' when payment_clear_status=3 then 'Cleared' when payment_clear_status=4 then 'Declined' when payment_clear_status=5 then 'Void' when payment_clear_status=6 then 'Ready to process Refund' when payment_clear_status=7 then 'Waiting Authorization' when payment_clear_status=8 then 'waiting posting' when payment_clear_status=9 then 'BACS Unpaid Resubmit' when payment_clear_status=10 then 'Call For Authorization' when payment_clear_status=11 then 'Ready to Auth (Failsoft)' when payment_clear_status=12 then 'Authorized (Failsoft)' END )as payment_clear_status,\r\n" + 
								"payment_clear_status as pay_status,payment_seq,payment_type,currency,pay_currency_amount,case when refund_deposit_pay_amt<>0 then refund_deposit_pay_amt else customer_deposit_pay_amt END customer_deposit_pay_amt,\r\n" + 
								"customer_deposit_pay_amt as deposit_balance,base_amount ,(pay_exchange_rate*case when refund_deposit_pay_amt<>0 then refund_deposit_pay_amt else customer_deposit_pay_amt END)as initial_base_amount,\r\n" + 
								"(pay_exchange_rate*customer_deposit_pay_amt)as deposit_balance_base from payment where customer_id ="+customerId+" and (customer_deposit_pay_amt != 0 or refund_deposit_pay_amt!= 0) and currency= '" + currency + "'");
			} else {
				depositDetails = jdbcTemplate.queryForList("select creation_date,(CASE when payment_clear_status=0 then 'Ready to Authorize' when payment_clear_status=1 then 'Authorized' when payment_clear_status=2 then 'Ready to Post' when payment_clear_status=3 then 'Cleared' when payment_clear_status=4 then 'Declined' when payment_clear_status=5 then 'Void' when payment_clear_status=6 then 'Ready to process Refund' when payment_clear_status=7 then 'Waiting Authorization' when payment_clear_status=8 then 'waiting posting' when payment_clear_status=9 then 'BACS Unpaid Resubmit' when payment_clear_status=10 then 'Call For Authorization' when payment_clear_status=11 then 'Ready to Auth (Failsoft)' when payment_clear_status=12 then 'Authorized (Failsoft)' END )as payment_clear_status,\r\n" + 
						"payment_clear_status as pay_status,payment_seq,payment_type,currency,pay_currency_amount,case when refund_deposit_pay_amt<>0 then refund_deposit_pay_amt else customer_deposit_pay_amt END customer_deposit_pay_amt,\r\n" + 
						"customer_deposit_pay_amt as deposit_balance,base_amount ,(pay_exchange_rate*case when refund_deposit_pay_amt<>0 then refund_deposit_pay_amt else customer_deposit_pay_amt END)as initial_base_amount,\r\n" + 
						"(pay_exchange_rate*customer_deposit_pay_amt)as deposit_balance_base from payment where customer_id ="+customerId+" and (customer_deposit_pay_amt !=0 or refund_deposit_pay_amt!=0)");
			}
	...
	@Override
	public List<TotalDepositSummaryModel> getTotalDepositList(int customerId, String currency) {
		List<TotalDepositSummaryModel> totalDepositDetails = new ArrayList<>();
		try {

			if (currency != null && currency != "" && !currency.toUpperCase().equals("[ALL]")) {
				List<Map<String, Object>> rows = jdbcTemplate.queryForList(
						"select sum(case when refund_deposit_pay_amt <>0 then refund_deposit_pay_amt else customer_deposit_pay_amt end) InitialDeposit,"
								+ "sum(customer_deposit_pay_amt) BalanceDeposit,sum(case when refund_deposit_pay_amt <>0 then refund_deposit_pay_amt* pay_exchange_rate "
								+ "else customer_deposit_pay_amt* pay_exchange_rate end) InitialDepositbase,"
								+ "sum(customer_deposit_pay_amt* pay_exchange_rate ) BalanceDepositbase, currency "
								+ "from view_payment_deposit  where customer_id=" + customerId
								+ " and (customer_deposit_pay_amt != 0 or refund_deposit_pay_amt != 0) group by currency");
				for (Map<String, Object> row : rows) {
					TotalDepositSummaryModel model = new TotalDepositSummaryModel();

					model.setInitialDeposit(row.get("InitialDeposit").toString());
					model.setBalanceDeposit(row.get("BalanceDeposit").toString());
					model.setInitialDepositbase(row.get("InitialDepositbase").toString());
					model.setBalanceDepositbase(row.get("BalanceDepositbase").toString());
					model.setCurrency(row.get("currency").toString());
					totalDepositDetails.add(model);
				}

			} else {
             Map<String, Object> allCurrencyMap = jdbcTemplate.queryForMap("select sum(case when refund_deposit_pay_amt <>0 then refund_deposit_pay_amt else customer_deposit_pay_amt end) InitialDeposit,"
						+ "sum(customer_deposit_pay_amt) BalanceDeposit,sum(case when refund_deposit_pay_amt <>0 then refund_deposit_pay_amt* pay_exchange_rate "
						+ "else customer_deposit_pay_amt* pay_exchange_rate end) InitialDepositbase,"
						+ "sum(customer_deposit_pay_amt* pay_exchange_rate ) BalanceDepositbase "
						+ "from view_payment_deposit  where customer_id =" + customerId
						+ " and (customer_deposit_pay_amt != 0 or refund_deposit_pay_amt != 0)");
              
                TotalDepositSummaryModel allModel = new TotalDepositSummaryModel();
				allModel.setInitialDeposit(allCurrencyMap.get("InitialDeposit").toString());
				allModel.setBalanceDeposit(allCurrencyMap.get("BalanceDeposit").toString());
				allModel.setInitialDepositbase(allCurrencyMap.get("InitialDepositbase").toString());
				allModel.setBalanceDepositbase(allCurrencyMap.get("BalanceDepositbase").toString());
				// allModel.setCurrency(allCurrencyMap.get("currency").toString());
	...
	public double fetchPayAmount(int customerId, int orderId, int bill_to_customer_id) {
		...
		Double rows = 0.0d;
		try {
			List<Map<String, Object>> orderItemtype = jdbcTemplate
					.queryForList("select distinct order_item_type from order_item where customer_id=" + customerId
							+ " and orderhdr_id=" + orderId);
			for (Map<String, Object> result : orderItemtype) {
				if (Integer.parseInt(result.get("order_item_type").toString()) == 7) {
					rows = jdbcTemplate.queryForObject(
							"select sum(gross_local_amount) as gross_base_amount from order_item where customer_id="
									+ customerId + " and "
									+ "pkg_item_seq is null and (order_item_type in (7) or payment_status not in (1,2,3,4) and gross_local_amount > 0.0 and payment_status in (0,5) and order_status in (0,5,7,8,9,10,11,12,13,15,16) and orderhdr_id="
									+ orderId + ")",
							Double.class);
				...
				if (orderId == 0) {
					...
					rows = jdbcTemplate.queryForObject(
							"select sum(gross_local_amount) as gross_base_amount  from order_item where bill_to_customer_id = "
									+ bill_to_customer_id + "and orderhdr_id =" + orderId
									+ "and  payment_status in (0,5)",
							Double.class);
				}
			}
		} catch (Exception e) {
	...
	public List<Map<String, Object>> getWithdrawalDetails(int customerId, String currency) {
			...
			if (currency != null && currency != "" && !currency.toUpperCase().equals("[ALL]")) {
				...
				withdrawlDetailsDetails = jdbcTemplate.queryForList(
						"SELECT p.customer_id, o.orderhdr_id, o.order_item_seq, oc.oc_id, d.creation_date as payment_date, p.currency as pay_currency,"
								...
								+ " and p.customer_id = " + customerId + "  and p.currency='" + currency
	...
	public boolean depositSave(PaymentModel paymentModel) {
		try {
			...
			if (String.valueOf(paymentModel.getPayment_account_seq()) == ""
					&& String.valueOf(paymentModel.getPayment_account_seq()) == null) {
				paymentParameters.put("payment_account_seq", 0);
			} else {
				paymentParameters.put("payment_account_seq", paymentModel.getPayment_account_seq());
			}
			...
			jdbcTemplate.update(
					"set nocount on if not exists (select 1 from information_schema.tables where table_name = 'mru_journal_id') begin create"
			...
			String PaymentSeqValue = jdbcTemplate.queryForObject(
					...
					String.class);
			if (PaymentSeqValue != null) {
				int mruPaymentSeq = Integer.parseInt(PaymentSeqValue) + 1;
				System.out.println(mruPaymentSeq);
				parametersToUpdate.put("mru_payment_seq", mruPaymentSeq);
			} else {
				parametersToUpdate.put("mru_payment_seq", 1);
			}
		...
		try {
			...
			querySearchLookupOrder.append(
					"select oi.orderhdr_id,oi.bill_to_customer_id as customer_id,oi.order_qty,oi.order_date,oi.refund_status,oi.currency, oc.order_code,oc.order_code_id,"
							+ "oi.order_item_seq,oi.payment_status,oi.customer_id,CONCAT(c.fname,' ' collate Latin1_General_CI_AS,c.lname)as fullname,oi.order_date,oi.order_status,oi.subscrip_id,"
		...
		try {
			...
			for (Map<String, Object> value : rows) {
				...
				if (tarnsactionType == 1) {
					String updatequery = "update view_order_tab_list_all set payment_status=0 where orderhdr_id=" + orderId
	...
	public List<Map<String, Object>> getPaymentThresholdData(int orderId, int orderItemSeq) {
		...
		term = jdbcTemplate
				.queryForList("select term from term where term_id=(select term_id from order_code where order_code_id="
						+ "(select order_code_id from order_item where orderhdr_id=" + orderId + "and order_item_seq="
						+ orderItemSeq + "))");
...
//		}
	...
	public String getUnderPaymentOption() {
		...
		try {
			...
			/*
			 * value = jdbcTemplate.
			 * queryForObject("select under_payment_notify from payment_threshold",String.
			 * class);
			 */
		} catch (Exception e) {
			LOGGER.error(ERROR + e);
	...
	public List<DropdownModel> currencyList(int customerId) {
		...
		try {
			List<Map<String, Object>> rows = jdbcTemplate.queryForList(
					"select currency from  payment  where  " + "customer_id=" + customerId + "  group by currency");
			for (Map<String, Object> row : rows) {
				DropdownModel model = new DropdownModel();
				model.setKey(row.get("currency").toString());
	...
	public List<DropdownModel> getDefaultPaymentTypeInfo() {
		...
		try {
			defaultValues = jdbcTemplate.queryForList(
					"  select config.default_payment_type_pay_add,payment_type.description from config inner join payment_type on payment_type.payment_type=config.default_payment_type_pay_add ");
			for (Map<String, Object> row : defaultValues) {
				DropdownModel model = new DropdownModel();
				model.setKey(row.get("default_payment_type_pay_add").toString() != null
						? row.get("default_payment_type_pay_add").toString()
						: "");
	...
	public List<DropdownModel> getPaymentType(int customer_id) {
		...
		try {
			defaultValues = jdbcTemplate.queryForList(
					" select payment_account.payment_type,payment_type.description,payment_type.payment_form,payment_type.credit_card_type from payment_account inner join payment_type on payment_account.payment_type=payment_type.payment_type where customer_id="
							+ customer_id);
			// defaultValues = jdbcTemplate.queryForList(" select top 1
			// payment_account.payment_type,payment_type.description,payment_type.payment_form,payment_type.credit_card_type
			// from payment_account inner join payment_type on
			// payment_account.payment_type=payment_type.payment_type where
	...
	public List<DropdownModel> getdefaultPaymentAccountSeqInMake(int payerCustomer) {
		...
		try {
			defaultValues = jdbcTemplate.queryForList(
					" select top 1 payment_account_seq,description,id_nbr_last_four from payment_account where customer_id ="
							+ payerCustomer);
			for (Map<String, Object> row : defaultValues) {
				DropdownModel model = new DropdownModel();
	...
	public String getCreditCardNumber(int customer_id, int payment_seq) {
		...
		try {
			...
			cardNumber = jdbcTemplate.queryForList(
					"select id_nbr from payment where customer_id=" + customer_id + " and payment_seq=" + payment_seq);
	...
	@Override
	public List<TotalDepositSummaryModel> getTotalWithDrawlDetails(int customerId, String currency) {

		List<TotalDepositSummaryModel> totalWithDrawlDetails = new ArrayList<>();

		try {
			if (currency != null && currency != "" && !currency.toUpperCase().equals("[ALL]")) {
				Map<String, Object> allWithDrawlMap = jdbcTemplate
						.queryForMap("select SUM(pay_currency_amount) as totalWithDrawlAmount  from payment  "
								+ "where customer_id=" + customerId + " and currency='" + currency + "'");
				TotalDepositSummaryModel allModel = new TotalDepositSummaryModel();
				allModel.setTotalWithDrawlAmount(allWithDrawlMap.get("totalWithDrawlAmount").toString());
				totalWithDrawlDetails.add(allModel);
			} else {
		...
		return totalWithDrawlDetails;
	}

	// code added by Malini to get EnableDisable value for payAmount field in editPayment page 04-04-2020
	@Override
	public Boolean getEnableDisablePayAmount(int customer_id, int payment_seq) {
		System.out.println("inside getEnableDisablePayAmount field api.......");
		List<Map<String, Object>> getValues = new ArrayList();
		boolean enableDisable = true;
		try {
			getValues = jdbcTemplate.queryForList(
					"select payment.payment_type,payment.transaction_type,payment.is_reversed,payment.customer_id,payment.payment_seq,"
							+ "payment.payment_clear_status,payment.processing,payment_type.payment_form  from payment_type inner join payment on payment_type.payment_type=payment.payment_type"
							+ " where customer_id=" + customer_id + " and payment_seq=" + payment_seq);
			int transactionType = Integer.parseInt(getValues.get(0).get("transaction_type").toString());
			int isReversed = Integer.parseInt(getValues.get(0).get("is_reversed").toString());
			int paymentClearStatus = Integer.parseInt(getValues.get(0).get("is_reversed").toString());
			int paymentForm = Integer.parseInt(getValues.get(0).get("payment_form").toString());
			int processing = Integer.parseInt(getValues.get(0).get("processing").toString());

			if (transactionType == 0 && isReversed == 0) {
				if ((paymentForm == 1) || (paymentForm == 4) || (paymentForm == 7)) {
					if ((paymentClearStatus == 3) || (paymentClearStatus == 4)) {
						enableDisable = false;
					} else if (paymentClearStatus == 2) {

						if (processing != 1) {
							enableDisable = false;
						}
					}
				} else if (paymentForm == 6) {
					enableDisable = false;
				}
				List<Map<String,Object>> listOfValues = jdbcTemplate.queryForList(
						"select pr.transaction_type,* from payment P Inner JOIn payment_reversed_payment R On R.original_customer_id=p.customer_id and R.original_payment_seq =P.payment_seq\r\n" + 
						"Inner JOIN payment pr on pr.customer_id =R.reversed_customer_id and pr.payment_seq =R.reversed_payment_seq " + 
						"where P.customer_id ="+customer_id+" and p.payment_seq ="+payment_seq+"and Pr.transaction_type =2" );
				int size=listOfValues.size();
				if (size != 0) {
					enableDisable = false;
				}

				List<Map<String,Object>> listOfValues1 = jdbcTemplate.queryForList(
						"select Distinct o.order_status from view_payment_tab_list_all p inner JOIn order_item o ON p.o_orderhdr_id=o.orderhdr_id "
								+ "where p.Customer_id=" + customer_id + " and p.payment_seq =" + payment_seq
								+ " and order_status in" + " (0,5,7,8,9,10,11,12,13,15,16,17) and pkg_item_seq is null");
				int ValuesSize=listOfValues1.size();
				if (ValuesSize ==0) {
					enableDisable = false;
				}
