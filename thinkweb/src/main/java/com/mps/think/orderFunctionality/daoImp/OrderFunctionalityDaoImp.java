...
public class OrderFunctionalityDaoImp implements OrderFunctionalityDao {
	...
	public List<SuspensionModel> getTemporarySuspension(long orderhdrId, int orderItemSeq) {
		...
		try {
			...
			if(order1.isEmpty() || order1.equals(null) || order1.equals("")) {
				...
				query.append(" from order_item left join suspension on order_item.orderhdr_id = suspension.orderhdr_id and order_item.order_item_seq = suspension.order_item_seq left join order_code on order_item.order_code_id = order_code.order_code_id left join customer on order_item.customer_id = customer.customer_id left join suspension_note sn on sn.orderhdr_id=order_item.orderhdr_id and sn.suspension_seq=suspension.suspension_seq where order_item.orderhdr_id=" + orderhdrId + " and order_item.order_item_seq="+ orderItemSeq );
	...
	public EditSuspension getAddTempSuspension(long orderhdrId, int orderItemSeq) {
		...
		try {
			...
			if (startIssueId == null) {
				query.append(" FORMAT (GETDATE(), 'dd/MM/yyyy') as start_date ,");  
			}else {
				int orderType = jdbcTemplate.queryForObject("select order_item_type from order_item where orderhdr_id = "+ orderhdrId + " and order_item_seq= " + orderItemSeq, Integer.class);
				if(orderType == 0) {
					...
					if ( issueDate == null) {
						query.append(" FORMAT ((select top 1 issue_date from issue where oc_id=(select oc_id from order_item  where orderhdr_id = "+ orderhdrId + " and order_item_seq= " + orderItemSeq + ")), 'dd/MM/yyyy') as start_date,");
					}else {
						query.append(" FORMAT ((SELECT DATEADD(day,1,(select top 1 issue_date from issue where oc_id= (select oc_id from order_item where order_item.orderhdr_id ="+ orderhdrId +" and order_item.order_item_seq= " + orderItemSeq +") and   issue_close_date=(select max(issue_close_date) from issue where oc_id=(select oc_id from order_item where order_item.orderhdr_id =" + orderhdrId + " and order_item.order_item_seq= " + orderItemSeq+")) order by issue_id desc ) ) AS DateAdd ), 'dd/MM/yyyy') as start_date,");
					}
				}else {
					query.append(" FORMAT (GETDATE(), 'dd/MM/yyyy') as start_date ,"); 
				}
			}
			query.append(" '' as expire_date,'1' as serve_backlabels, description, order_item.user_code as user_code from order_item left join order_code on order_item.order_code_id=order_code.order_code_id left join suspension on order_item.orderhdr_id=suspension.orderhdr_id and order_item.order_item_seq=suspension.order_item_seq left join customer on order_item.customer_id=customer.customer_id  where order_item.orderhdr_id =" + orderhdrId + " and order_item.order_item_seq = " + orderItemSeq);	
			...
			int orderType = jdbcTemplate.queryForObject("select order_item_type from order_item where orderhdr_id="+orderhdrId+" and order_item_seq= "+orderItemSeq, Integer.class);
			int subscrip = jdbcTemplate.queryForObject("select subscrip_start_type from order_item where orderhdr_id="+orderhdrId+" and order_item_seq= "+orderItemSeq, Integer.class);
			for(Map<String, Object> tempSuspension:addTempSuspensionList){	
				...
				if(orderType ==0 || orderType == 1) {
					if(subscrip == 0) {
						editSuspension.setAllow(1);
					}else {
						editSuspension.setAllow(0);
					}
				}else {
					editSuspension.setAllow(0);
				}
	...
	public int saveAddTempSuspension(SuspensionModel suspensionModel) {
		...
		try {
			...
			StringBuilder query = new StringBuilder("select * from order_item where orderhdr_id=" + suspensionModel.getOrderhdrId()+ " and order_item_seq=" + suspensionModel.getOrderItemSeq());
			...
			String issue = jdbcTemplate.queryForObject("select start_issue_id from order_item where orderhdr_id="+suspensionModel.getOrderhdrId()+" and order_item_seq="+suspensionModel.getOrderItemSeq(), String.class);
			if(issue == null) {
				addParams.put("begin_issue_id", null);	
	...
	public EditSuspension getEditSuspension(long orderhdrId, int orderItemSeq, int suspensionSeq) {
		...
		try {
			...
			query.append("  select top 1 (case when customer.fname is null then CONVERT(nvarchar(10),customer.customer_id) else customer.fname + ' ' + lname end) AS customer, "
					...
					+ " FORMAT (suspend_from_date, 'dd/MM/yyyy') as suspend_from_date,FORMAT (suspend_to_date, 'MM/dd/yyyy')  as suspend_to_date,serve_backlabels,n_backlabels,"
					...
					+ " left join suspension on order_item.orderhdr_id=suspension.orderhdr_id and order_item.order_item_seq=suspension.order_item_seq "
				...
				for(Map<String, Object> tempSuspension:editSuspensionList){	
					...
					editSuspension.setAlwaysSendBackissues(alwaysSendIssue);
					int suspend = jdbcTemplate.queryForObject("select suspended_order_status from suspension where orderhdr_id = " + orderhdrId + " and order_item_seq=" + orderItemSeq+ " and suspension_seq=" + suspensionSeq, Integer.class);
					if(suspend == 8 || suspend == 10) {
						if(orderType ==0 ) {
							if(subscrip == 0) {
								editSuspension.setAllow(1);
							}else {
	...
	public int updateEditSuspension(SuspensionModel suspensionModel) {
		...
		int status = 0;
		try {
			int suspendedOrder = jdbcTemplate.queryForObject("select suspended_order_status from suspension where orderhdr_id=" + suspensionModel.getOrderhdrId()+ " and order_item_seq=" + suspensionModel.getOrderItemSeq() +" and suspension_seq="+suspensionModel.getSuspensionSeq() +" order by orderhdr_id,order_item_seq desc", Integer.class);
			if(suspendedOrder == 9) {
	...
	public EditSuspension getReinstateOrderItem(long orderhdrId, int orderItemSeq, int suspensionSeq) {
		...
		try {
				StringBuilder query = new StringBuilder(" select CONCAT(fname, ' ' collate Latin1_General_CI_AS, lname) AS fname,order_code.description as description,");
				int prePayment = jdbcTemplate.queryForObject("select prepayment_req from order_item where orderhdr_id = " + orderhdrId + " and order_item_seq=" + orderItemSeq , Integer.class);
				if(prePayment == 1) {
					int orderStatus = jdbcTemplate.queryForObject("select old_order_status from suspension where orderhdr_id = " + orderhdrId + " and order_item_seq=" + orderItemSeq + " and suspension_seq= " + suspensionSeq, Integer.class);
					if(orderStatus == 8 || orderStatus == 9 || orderStatus == 10 || orderStatus ==16) {
						query.append("(select count(*) from suspension where orderhdr_id = " + orderhdrId + " and order_item_seq=" + orderItemSeq + " and suspension_status!=0 and suspension_status!=3 and suspension_status!=4) as active,");
					}else {
						query.append(" order_code.active as active, ");
					}
				}else {
					query.append(" order_code.active as active, ");
				}
				query.append(" suspended_order_status, old_order_status,CONVERT(VARCHAR, start_date, 101) as start_date,CONVERT(VARCHAR, expire_date, 101) as expire_date,subscrip.start_issue_id as start_issue_id, '0' as back_issue,suspension_seq,suspension_status,(select CONVERT(varchar,CONVERT(DATE,suspension.creation_date,101),101)) as creation_date, subscrip.stop_issue_id as stop_issue_id,serve_backlabels,n_backlabels,customer.user_code from order_item left join suspension on order_item.orderhdr_id=suspension.orderhdr_id and order_item.order_item_seq=suspension.order_item_seq left join order_code on order_item.order_code_id=order_code.order_code_id left join subscrip on order_item.subscrip_id=subscrip.subscrip_id left join customer on order_item.customer_id=customer.customer_id where order_item.orderhdr_id = " + orderhdrId + " and order_item.order_item_seq=" + orderItemSeq+ " and suspension_seq=" + suspensionSeq);
	...
	public int updateReinstateOrderItem(SuspensionModel suspensionModel) {
		...
		try {
			...
			int orderStatus = jdbcTemplate.queryForObject("select old_order_status from suspension where orderhdr_id="+ suspensionModel.getOrderhdrId() +" and order_item_seq="+ suspensionModel.getOrderItemSeq() +" and suspension_seq="+suspensionModel.getSuspensionSeq(), Integer.class);
			StringBuilder orderItem = new StringBuilder("update order_item set order_status=:order_status where orderhdr_id=" + suspensionModel.getOrderhdrId()+ " and order_item_seq=" + suspensionModel.getOrderItemSeq());
			updateParams.put("order_status",  orderStatus);
			...
			int prePayment = jdbcTemplate.queryForObject("select prepayment_req from order_code where order_code_id=(select order_code_id from order_item where orderhdr_id = " + suspensionModel.getOrderhdrId() + " and order_item_seq=" + suspensionModel.getOrderItemSeq() + ")", Integer.class);
			if(suspendedOrder == 9 || suspendedOrder == 16) {
				StringBuilder suspension = new StringBuilder("");
				suspension.append("update suspension set suspension_status=:suspension_status, suspend_to_date=:suspend_to_date, serve_backlabels=:serve_backlabels,end_issue_id=:end_issue_id where orderhdr_id="+ suspensionModel.getOrderhdrId() +" and order_item_seq="+ suspensionModel.getOrderItemSeq() +" and suspension_seq="+suspensionModel.getSuspensionSeq());
				if(prePayment == 1) {
					if(orderStatus == 8 || orderStatus == 9 || orderStatus == 10 || orderStatus ==16) {
						updateParams.put("suspension_status",  2);
					}else {
						updateParams.put("suspension_status",  3);
					}
				}else {
					updateParams.put("suspension_status",  3);
				}
				updateParams.put("end_issue_id",  order.get(0).get("start_issue_id"));
				...
				suspension1.append("update suspension set suspension_status=:suspension_status, suspend_to_date=:suspend_to_date, serve_backlabels=1 ,end_issue_id=:end_issue_id where orderhdr_id="+ suspensionModel.getOrderhdrId() +" and order_item_seq="+ suspensionModel.getOrderItemSeq() );
				if(prePayment == 1) {
					if(orderStatus == 8 || orderStatus == 9 || orderStatus == 10 || orderStatus ==16) {
						updateParams.put("suspension_status",  2);
					}else {
	...
	public Map<String, Object> getExtSubscripChange(long orderhdrId, int orderItemSeq, int count) {
		...
		try {
			...
			if (revenueMethod == 0) {
				...
				StringBuilder query = new StringBuilder(" SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY issue_date ASC) AS rownumber,issue_id,CONVERT(VARCHAR, issue_date, 101) as issue_date FROM issue where oc_id = (select oc_id from order_item where orderhdr_id="+orderhdrId+" and order_item_seq="+orderItemSeq+") and issue_id>= (select start_issue_id from order_item where orderhdr_id="+orderhdrId+" and order_item_seq="+orderItemSeq+") and issue_close_date is null ) as temptablename WHERE rownumber >(select n_remaining_paid_issues -1 from order_item where orderhdr_id="+orderhdrId+" and order_item_seq="+orderItemSeq+")");
	...
	public int updateExtendSubscription(long orderhdrId, int orderItemSeq, int count) {
		...
		try {
			...
			if (revenueMethod == 0) {
				List<Map<String, Object>> issueList = new ArrayList<>();
				StringBuilder query1 = new StringBuilder(" SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY issue_date ASC) AS rownumber,issue_id,CONVERT(VARCHAR, issue_date, 101) as issue_date FROM issue where oc_id = (select oc_id from order_item where orderhdr_id="+orderhdrId+" and order_item_seq="+orderItemSeq+") and issue_id>= (select start_issue_id from order_item where orderhdr_id="+orderhdrId+" and order_item_seq="+orderItemSeq+") and issue_close_date is null ) as temptablename WHERE rownumber >(select n_remaining_paid_issues -1 from order_item where orderhdr_id="+orderhdrId+" and order_item_seq="+orderItemSeq+")");
	...
	public EditSuspension getReinstateBehavioralSuspension(long orderhdrId, int orderItemSeq,int suspensionSeq) {
		...
		try {
			int status = jdbcTemplate.queryForObject("select suspension_status from suspension where orderhdr_id = " + orderhdrId + " and order_item_seq=" + orderItemSeq + " and suspension_seq= " + suspensionSeq,Integer.class);
			if (status == 1) {
				StringBuilder query = new StringBuilder("select (case when customer.fname is null then CONVERT(nvarchar(10),customer.customer_id) else customer.fname + ' ' + lname end) AS fname, CONVERT(varchar,CONVERT(DATE,suspension.creation_date,101),101) as creation_date,suspension_status, order_code.description, suspended_order_status, old_order_status, ");
				int prePayment = jdbcTemplate.queryForObject("select prepayment_req from order_item where orderhdr_id = " + orderhdrId + " and order_item_seq=" + orderItemSeq , Integer.class);
				if(prePayment == 1) {
					int orderStatus = jdbcTemplate.queryForObject("select old_order_status from suspension where orderhdr_id = " + orderhdrId + " and order_item_seq=" + orderItemSeq + " and suspension_seq= " + suspensionSeq, Integer.class);
					if(orderStatus == 8 || orderStatus == 9 || orderStatus == 10 || orderStatus ==16) {
	...
	public int savePayHoldSuspension(SuspensionModel suspensionModel) {
		...
		try {
			...
			List<String> subscrip = jdbcTemplate.queryForList("select start_issue_id from order_item where orderhdr_id=" + suspensionModel.getOrderhdrId()+ " and order_item_seq=" + suspensionModel.getOrderItemSeq(), String.class);
			String subId = null;
			if(subscrip.size() != 0)
			...
			if(subId != null) {
				...
				String addDel = jdbcTemplate.queryForObject("select max(additions_deletions_seq) from additions_deletions where subscrip_id="+order.get(0).get("subscrip_id"), String.class);
				addParams.put("mru_additions_deletions_seq", addDel);
	...
	public int saveNonPaymentSuspension(SuspensionModel suspensionModel) {
		...
		try {
			...
			StringBuilder query = new StringBuilder("select * from order_item where orderhdr_id=" + suspensionModel.getOrderhdrId()+ " and order_item_seq=" + suspensionModel.getOrderItemSeq()+" and order_status in (0,5,7,8,9,10,11,12,13)");
			orderDetail = jdbcTemplate.queryForList(query.toString());
			...
			if(!order.isEmpty()) {
				...
				if(susOrder.equals("10") && susStatus.equals("1")) {
					int prePayment = jdbcTemplate.queryForObject("select prepayment_req from order_item where orderhdr_id = " + suspensionModel.getOrderhdrId() + " and order_item_seq=" + suspensionModel.getOrderItemSeq() , Integer.class);
					if(prePayment == 1) {
						int suspSeq = jdbcTemplate.queryForObject("select max(suspension_seq) from suspension where orderhdr_id = " + suspensionModel.getOrderhdrId() + " and order_item_seq=" + suspensionModel.getOrderItemSeq() , Integer.class);
						if(suspSeq == 1) {
							...
							String issue = jdbcTemplate.queryForObject("select start_issue_id from order_item where orderhdr_id="+suspensionModel.getOrderhdrId()+" and order_item_seq="+suspensionModel.getOrderItemSeq(), String.class);
							if(issue == null) {
								addParams.put("end_issue_id", null);
							}else if(issue.equals("0")) {
								addParams.put("end_issue_id", null);
							}else {
								addParams.put("end_issue_id", orderDetail.get(0).get("start_issue_id"));
							}
							addParams.put("orderhdr_id", suspensionModel.getOrderhdrId());
							addParams.put("order_item_seq", suspensionModel.getOrderItemSeq());
							String maxSeq = jdbcTemplate.queryForObject("select max(suspension_seq) from suspension where orderhdr_id=" + suspensionModel.getOrderhdrId() + " and order_item_seq ="+ suspensionModel.getOrderItemSeq(),String.class);
							addParams.put("suspension_seq", maxSeq);
							namedParameterJdbcTemplate.update(suspension.toString(), addParams);
							
							StringBuilder addQuery = new StringBuilder("insert into suspension (orderhdr_id,order_item_seq,suspension_seq,subscrip_id,user_code,creation_date,suspension_status,old_order_status,suspended_order_status,suspend_from_date,begin_issue_id,serve_backlabels,n_backlabels,rollback_begin_issue_id, rollback_end_issue_id, rollback_job_id, rollback_n_backlabels, rollback_old_order_status, rollback_serve_backlabels, rollback_suspend_from_date, rollback_suspend_to_date, rollback_suspension_status, mru_suspension_note_seq ) values (:orderhdr_id,:order_item_seq,:suspension_seq,:subscrip_id,:user_code,:creation_date,:suspension_status,:old_order_status,:suspended_order_status,:suspend_from_date,:begin_issue_id,:serve_backlabels,:n_backlabels,:rollback_begin_issue_id, :rollback_end_issue_id, :rollback_job_id, :rollback_n_backlabels, :rollback_old_order_status, :rollback_serve_backlabels, :rollback_suspend_from_date, :rollback_suspend_to_date, :rollback_suspension_status, :mru_suspension_note_seq )");
							addParams.put("orderhdr_id", suspensionModel.getOrderhdrId());
							addParams.put("order_item_seq", suspensionModel.getOrderItemSeq());
							String maxSeq1 = jdbcTemplate.queryForObject("select max(suspension_seq) from suspension where orderhdr_id=" + suspensionModel.getOrderhdrId() + " and order_item_seq ="+ suspensionModel.getOrderItemSeq(),String.class);
							if (maxSeq1 == null) {
								addParams.put("suspension_seq", 1);
							} else {
								addParams.put("suspension_seq", Integer.parseInt(maxSeq1) + 1);
							}
							addParams.put("user_code", orderDetail.get(0).get("user_code"));
							addParams.put("creation_date", formatYYYYMMDDHHMMSS.format(new Date()));
							addParams.put("subscrip_id", orderDetail.get(0).get("subscrip_id"));
							addParams.put("suspension_status", 2);
							addParams.put("old_order_status", orderDetail.get(0).get("order_status"));
							...
							if(issue == null) {
								...
								addParams.put("begin_issue_id", orderDetail.get(0).get("start_issue_id"));
							}
							addParams.put("rollback_begin_issue_id", null);
							addParams.put("rollback_end_issue_id", null);
							addParams.put("rollback_job_id", null);
							...
							StringBuilder editTrail = new StringBuilder("insert into edit_trail (edit_trail_id,customer_id,customer_address_seq,user_code,subscrip_id,orderhdr_id,order_item_seq,edited,currency,table_enum,document_reference_id,local_amount,base_amount,date_stamp,creation_date,xaction_name)"
											+ "values (:edit_trail_id,:customer_id,:customer_address_seq,:user_code,:subscrip_id,:orderhdr_id,:order_item_seq,:edited,:currency,:table_enum,:document_reference_id,:local_amount,:base_amount,:date_stamp,:creation_date,:xaction_name)");
							addParams.put("edit_trail_id", editTrailId);
							addParams.put("customer_id", orderDetail.get(0).get("customer_id"));
							addParams.put("customer_address_seq", 1);
							addParams.put("user_code", orderDetail.get(0).get("user_code"));
							addParams.put("subscrip_id", orderDetail.get(0).get("subscrip_id"));
							addParams.put("orderhdr_id", suspensionModel.getOrderhdrId());
							addParams.put("order_item_seq", suspensionModel.getOrderItemSeq());
							addParams.put("edited", 1);
							addParams.put("currency", orderDetail.get(0).get("currency"));
							addParams.put("table_enum", 3);
							addParams.put("document_reference_id", 1);
							addParams.put("local_amount", orderDetail.get(0).get("gross_local_amount"));
							addParams.put("base_amount", orderDetail.get(0).get("gross_base_amount"));
							addParams.put("date_stamp", dateStamp);
							addParams.put("creation_date", formatYYYYMMDDHHMMSS.format(new Date()));
							addParams.put("xaction_name", "suspension_add_request");
							namedParameterJdbcTemplate.update(editTrail.toString(), addParams);
							...
							if(subId != null) {
								StringBuilder addDelete = new StringBuilder("insert into additions_deletions (subscrip_id,additions_deletions_seq,orderhdr_id,order_item_seq,issue_id,customer_id,n_addition_copies,n_deletion_copies,sub_add,sub_kill,sub_kill_reason,sub_on,sub_off,sub_off_reason,sub_start,sub_stop,sub_stop_reason,add_kill_status,creation_date,bundle_qty,start_stop_status) values (:subscrip_id,:additions_deletions_seq,:orderhdr_id,:order_item_seq,:issue_id,:customer_id,:n_addition_copies,:n_deletion_copies,:sub_add,:sub_kill,:sub_kill_reason,:sub_on,:sub_off,:sub_off_reason,:sub_start,:sub_stop,:sub_stop_reason,:add_kill_status,:creation_date,:bundle_qty,:start_stop_status)");
								addParams.put("subscrip_id", orderDetail.get(0).get("subscrip_id"));
								String addDelSeq = jdbcTemplate.queryForObject("select max(additions_deletions_seq) from additions_deletions where subscrip_id="+orderDetail.get(0).get("subscrip_id"), String.class);
								if (addDelSeq == null) {
									addParams.put("additions_deletions_seq", 1);
								} else {
									addParams.put("additions_deletions_seq", Integer.parseInt(addDelSeq) + 1);
								...
								if(issue == null) {
									...
									addParams.put("issue_id", orderDetail.get(0).get("start_issue_id"));
								}
								...
								String addDel = jdbcTemplate.queryForObject("select max(additions_deletions_seq) from additions_deletions where subscrip_id="+orderDetail.get(0).get("subscrip_id"), String.class);
								addParams.put("mru_additions_deletions_seq", addDel);
								addParams.put("subscrip_id", orderDetail.get(0).get("subscrip_id"));
			...
			if(susOrder.equals("8") && susStatus.equals("2")) {
				int prePayment = jdbcTemplate.queryForObject("select prepayment_req from order_item where orderhdr_id = " + suspensionModel.getOrderhdrId() + " and order_item_seq=" + suspensionModel.getOrderItemSeq() , Integer.class);
				if(prePayment == 1) {
					int suspSeq = jdbcTemplate.queryForObject("select max(suspension_seq) from suspension where orderhdr_id = " + suspensionModel.getOrderhdrId() + " and order_item_seq=" + suspensionModel.getOrderItemSeq() , Integer.class);
					if(suspSeq == 2) {
			...
			if(subId != null) {
				...
				StringBuilder subscripQuery = new StringBuilder("update subscrip set mru_additions_deletions_seq=:mru_additions_deletions_seq where subscrip_id=:subscrip_id");
				String addDel = jdbcTemplate.queryForObject("select max(additions_deletions_seq) from additions_deletions where subscrip_id="+subscripId, String.class);
				addParams.put("mru_additions_deletions_seq", addDel);
				addParams.put("subscrip_id", subscripId);
				namedParameterJdbcTemplate.update(subscripQuery.toString(), addParams);
				addParams.clear();
	...
	public int saveTransferOrder(TransferOrderModel transferOrderModel) {
		...
		try {
			...
			String name = jdbcTemplate.queryForObject("select distinct concat(fname,' ' collate Latin1_General_CI_AS,lname) as name from customer where customer_id="+transferOrderModel.getCustomerId(), String.class);
			...
			saveParams.put("creation_date", formatYYYYMMDDHHMMSS.format(new Date()));
			String name2 = jdbcTemplate.queryForObject("select distinct concat(fname,' ' collate Latin1_General_CI_AS,lname) as name from customer where customer_id="+custId, String.class);
		 ...
		 @Override
			public CustomerGroupModel getGroup(int customerId, int customerAddressSeq) {
				...
				try {
					StringBuilder query = new StringBuilder();
					query.append(
							" select customer_group,description,rep_customer_id,rep_customer_address_seq,address1,(select concat(fname,' ' collate Latin1_General_CI_AS,lname) from customer_address where customer_id="
									...
									+ "(select renew_to_customer_id from customer_group where customer_id=" + customerId + " and customer_address.customer_address_seq=" + customerAddressSeq+ ") and customer_address.customer_address_seq=" + customerAddressSeq +")as renew_to_address,bill_to_customer_id,bill_to_customer_address_seq,(select concat(fname,' ' collate Latin1_General_CI_AS,lname) from customer_address where customer_id="
	...
	public ProspectModel getEditProspectSetup(long customerId, int customerProspectSeq){
		...
		try {
			...
			query.append(" isNull(source_code,'') as source_code,isNull(source_code.description,'') as source_description, "  
					 + " isNull(concat(fname,' ' collate Latin1_General_CI_AS,lname),'') as referred_by,referred_by_customer_id, "
...
public List<Map<String, Object>> getAddrStatus(int customerId) {
	List<Map<String, Object>> addressStatus = null;
	try {
		String query="select address_undeliverable from address_status left join customer_address ca on ca.address_status = address_status.address_status where customer_id="+customerId+" and address_undeliverable=0";	
		addressStatus = jdbcTemplate.queryForList(query.toString());
		
		//addressStatus = jdbcTemplate.queryForObject("select address_undeliverable as address from address_status left join customer_address ca on ca.address_status = address_status.address_status where customer_id="+customerId+" and address_undeliverable=0", String.class);
	} catch (Exception e) {
		LOGGER.error(ERROR + e);
	}
...
public List<Map<String, Object>> getGroupMemberEnable(int customerId) {
	List<Map<String, Object>> grpMdrEnable = null;
	try {
		String query="select * from group_member where customer_id="+customerId;	
		grpMdrEnable = jdbcTemplate.queryForList(query.toString());
