...
import com.mps.think.setup.resultMapper.sourceCodeDetailMapper;
...
public class OrderClassDaoImpl implements OrderClassDetailsDao {
	 ...
	 * this parameter is to be use for rate,discount class, source code, subscriptionDef,product & pkg def search.
	 * @author Chaman.Bharti
	 */
	public static final String rate = "rate_class";// to search rate class
	public static final String discount = "discount_class";// to search discount class
	public static final String source_code = "source_code";// for getDefaultSourceCode(String ocId)
	public static final String searchSourceCode = "searchSourceCode";//to search source code
	
	/**
	 * This isNullOrEmpty function is to handle null,0,"" for String type parameter to avoid NullPointerException,NumberFormatException
	 */
	Predicate<String> isNotNullOrEmpty = s-> (s != null && !s.equals("0")) && !s.trim().isEmpty();
	BiPredicate<String,String> isNotNullOrEmpty2 = (s1,s2) -> ((s1 != null && !s1.equals("0")) && !s1.trim().isEmpty()) && ((s2 != null && !s2.equals("0")) && !s2.trim().isEmpty());
	BiPredicate<String,String> isEquals = (a,b) -> a.equals(b);
	BiPredicate<Integer,Integer> isNotNull = (s1,s2) ->  s1!=null && s2 !=null;
	...
	public List<DropdownModel> getDefaultSourceCode(String ocId) {
		...
		try {
			...
			StringBuilder query = new StringBuilder("SELECT source_code_id,source_code,description");
//			query.append(",source_code_type,agency_customer_id,source_format,state_break,generated,breakeven,offer,list,from_date,to_date,qty,");
//			query.append("cost,audit_qual_category,generic_agency,oc_id,active,newsub_rate_class_id,new_renewal_card_id,newsub_discount_class_id,audit_subscription_type_id,");
//			query.append("audit_qual_source_id,audit_name_title_id,audit_sales_channel_id,row_version,premium_order_code_id,source_code_type,currency,is_ddp,shipping_price_list ");
			query.append(" FROM source_code WHERE ");
			if(isNotNullOrEmpty.test(ocId)) {
				appendOcId(Long.valueOf(ocId), query, source_code);
			}
			query.append(" AND source_code.active = 1 AND (GETDATE() BETWEEN source_code.from_date AND source_code.to_date ");
			query.append("OR (source_code.to_date IS NULL OR source_code.from_date IS NULL)) and (source_code_type = 1 OR source_code_type = 2) ORDER BY source_code");
			SourceCode = jdbcTemplate.queryForList(query.toString());
			for (Map<String, Object> sourceCode : SourceCode) {
				...
				model.setDescription(sourceCode.get("description") != null ? (String) sourceCode.get("description") : "");
...
//			 namedParameterJdbcTemplate.update(discountClassEffective, addOrderClassParams);
	...
	public int saveRateCardRecords(RateClassModel rateCard) {
		...
		try {
			if(rateCard.getRate_class_effective_seq()!=0) {
				rateClassEffectiveSeq=rateCard.getRate_class_effective_seq();
			}
			String maxPKId = jdbcTemplate.queryForObject(
					"select MAX(ratecard_seq) from ratecard where rate_class_id=" + rateCard.getRateClassId() +" and rate_class_effective_seq="+rateClassEffectiveSeq+"",
					String.class);
			if (maxPKId == null) {
			...
			saveRateCardParams.put("rate_class_effective_seq", rateClassEffectiveSeq);
	...
	public void updateMru_rate_card_seq(Integer rateClassId, Integer rateClassEffectiveSeq) {
		...
		try {
			...
			String mru_ratecard_seq = jdbcTemplate.queryForObject(
					"select max(mru_ratecard_seq) from rate_class_effective where rate_class_id=" + rateClassId +" and rate_class_effective_seq=" + rateClassEffectiveSeq,
					String.class);
	...
	public List<sourceCodeModel> getSourceCodeDetails(Long ocId, int active, String sourceCodeType, int isDDp, String sourceCode) {
		List<sourceCodeModel> SourceCodeTableData = new ArrayList<>();
		try {
			if((sourceCodeType.equals("") || sourceCodeType.isEmpty()) && (sourceCode.equals("") || sourceCode.isEmpty())) {
				SourceCodeTableData = namedParameterJdbcTemplate.query("select * from source_code where oc_id =" + ocId + " and active=" + active + " and is_ddp= " + isDDp,new sourceCodeDetailMapper());
			}else if((!sourceCodeType.equals("") || !sourceCodeType.isEmpty()) | (!sourceCode.equals("") || !sourceCode.isEmpty())){
				if(!sourceCodeType.equals("") || !sourceCodeType.isEmpty()) {
					if("*".equals(sourceCode)){
						SourceCodeTableData = namedParameterJdbcTemplate.query("select * from source_code where oc_id =" + ocId + " and active=" + active + " and source_code_type = "+sourceCodeType+"and source_code LIKE '%' ",new sourceCodeDetailMapper());
					}else if (sourceCode.contains("*")) {
						SourceCodeTableData = namedParameterJdbcTemplate.query("select * from source_code where oc_id =" + ocId + " and active=" + active + " and source_code_type = "+sourceCodeType+" and source_code LIKE '"+sourceCode.replace('*','%')+"' ",new sourceCodeDetailMapper());
					} else {
						SourceCodeTableData = namedParameterJdbcTemplate.query("select * from source_code where oc_id =" + ocId + " and active=" + active + " and source_code_type = "+sourceCodeType+" and source_code LIKE '"+sourceCode+"' ",new sourceCodeDetailMapper());
					}
				}else {
					if("*".equals(sourceCode)){
						SourceCodeTableData = namedParameterJdbcTemplate.query("select * from source_code where oc_id =" + ocId + " and active=" + active + " and source_code LIKE '%' ",new sourceCodeDetailMapper());
					}else if (sourceCode.contains("*")) {
						SourceCodeTableData = namedParameterJdbcTemplate.query("select * from source_code where oc_id =" + ocId + " and active=" + active + " and source_code LIKE '"+sourceCode.replace('*','%')+"' ",new sourceCodeDetailMapper());
					} else {
						SourceCodeTableData = namedParameterJdbcTemplate.query("select * from source_code where oc_id =" + ocId + " and active=" + active + " and and source_code LIKE '"+sourceCode+"' ",new sourceCodeDetailMapper());
					}
				}
				
			}else {
				SourceCodeTableData = namedParameterJdbcTemplate.query("select * from source_code where oc_id =" + ocId + " and active=" + active + " and source_code_type = "+sourceCodeType+" and is_ddp= " + isDDp,new sourceCodeDetailMapper());
			}
	...
	public List<ParentOrderClassModel> getSourceCodeDetails1(String  sourceCodeId) {
		...
		StringBuilder query = new StringBuilder("select source_code_id,agency_customer_id,agency_code,company,source_code.source_format,source_format.description as sourceDesc,source_code,"
				+ "source_code.description,state_break,generated,breakeven,offer,list,from_date,to_date,qty,cost,audit_qual_category,"
				+ "audit_subscription_type_id,audit_qual_source_id,audit_name_title_id,audit_sales_channel_id,generic_agency,"
				+ "source_code.oc_id, active,newsub_rate_class_id,rate_class,rate_class.description as rateDesc,new_renewal_card_id,"
				+ "renewal_card,renewal_card.description as renewalDesc, newsub_discount_class_id,discount_class,discount_class."
				+ "description as discountDesc,premium_order_code_id, source_code_type, mru_catalog_content_seq,currency,"
				+ "mru_generic_promotion_code_seq,is_ddp,source_code.shipping_price_list,shipping_price_list.description as shipDesc from source_code "
				+ "left join agency on agency.customer_id = source_code.agency_customer_id "
				+ "left join rate_class on rate_class.rate_class_id = source_code.newsub_rate_class_id "
				+ "left join discount_class on discount_class.discount_class_id = source_code.newsub_discount_class_id "
				+ "left join renewal_card on renewal_card.renewal_card_id = source_code.new_renewal_card_id "
				+ "left join shipping_price_list on shipping_price_list.shipping_price_list = source_code.shipping_price_list  left join source_format on source_format.source_format = source_code.source_format where source_code_id = "+sourceCodeId);
		try {
	...
	public List<Map<String, Object>> getEffectiveDiscountData(int discountId) {
		...
		try {
			EffectiveDateDiscountRateDetails = jdbcTemplate
					.queryForList("select discount_class_id,disc_class_effective_seq,description,effective_date+1 as effective_date ,renewal_expire_effective_date+1 as renewal_expire_effective_date,mru_discount_card_seq from disc_class_effective where discount_class_id =" + discountId);
		} catch (Exception e) {
			LOGGER.error(ERROR + e);
		}
...
//main menu
	...
	public List<UpSellModel> getUpSellTableData(int ocId) {
		...
		try {
			upSellTableData = jdbcTemplate.query("select * from upsell where oc_id="+ocId, new UpsellMapper());
	...
	/**
	 * To search rate class by oc id
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchRateClass(Long ocId) {
		List<Map<String, Object>> rateClassDetails = new ArrayList<>();
		StringBuilder query = new StringBuilder("SELECT oc.oc,rate_class.* FROM rate_class inner join oc on rate_class.oc_id = oc.oc_id WHERE ");
		int status = 0;
		try {
				appendOcId(ocId, query, rate);
				status++;
				if (status != 0) {
	 ...
	 * this method appends all ocId and parent oc id into query to search rate & discount class	
	 * @author Chaman.Bharti
	 */
	 ...
	 * @author Chaman.Bharti
	 */
	public void appendOcId(Long ocId, StringBuilder query, String action) {
		List<OrderClassModel> elementList = new ArrayList<>();
		try {
			  elementList = jdbcTemplate.query("select oc_id,parent_oc_id,oc,oc_type,case when oc_id = (select max(oc_id) from oc) then 'true' else 'false' end as IsActive from oc where oc_id = ?",
					new Object[] { ocId }, new OrderClassMapper());
			...
			for (OrderClassModel element : elementList) {
				...
				if (ocId != null && ocId != 0) {
					...
					case "discount_class":
						...
						break;
					case "source_code":
						query.append(" source_code.oc_id in (" + ocId);
						break;
					case "searchSourceCode":
						query.append(" and source_code.oc_id in (" + ocId);
						break;
	...
	/**
	 * To search rate class by rateClass, in this case oc id is mandatory
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchRateClass(Long ocId, String rateClass) {
		...
		int status = 0;
		try {
			 if (isNotNullOrEmpty.test(rateClass)) {
				//search by * wild card & rateClass
				 if(rateClass.contains("*")) {
					 query.append(" rate_class.rate_class like '").append(rateClass.replace('*', '%')).append("' and");
				 }else {
					 query.append(" rate_class.rate_class='").append(rateClass).append("' and");
				 }
				appendOcId(ocId, query, rate);
				status++;
			}
	...
	/**
	 * To search rate class by rateDescription, in this case oc id is mandatory
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchRateClass(long ocId, String rateDescription) {
		...
		int status = 0;
		try {
			 if (isNotNullOrEmpty.test(rateDescription)) {
				//search by * wild card & rateDescription
				 if(rateDescription.contains("*")) {
					 query.append(" rate_class.description like '").append(rateDescription.replace('*', '%')).append("' and");
				 }else {
					 query.append(" rate_class.description='").append(rateDescription).append("' and");
				 }
				appendOcId(ocId, query, rate);
				status++;
			}
	...
	/**
	 * To search rate class by rateClass & rateDescription, in this case oc id is mandatory
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchRateClass(Long ocId, String rateClass, String rateDescription) {
		...
		int status = 0;
		try {
			  //search by * wild card & rateClass,rateDescription
			  if (isNotNullOrEmpty2.test(rateClass,rateDescription)) {
				  if(rateClass.contains("*")) {
					  query.append(" rate_class.rate_class like '").append(rateDescription.replace('*', '%')).append("' and"); 
				  }else {
					  query.append(" rate_class.rate_class ='").append(rateDescription).append("' and"); 
				  } 
				  if(rateDescription.contains("*")) {
					  query.append(" rate_class.description like '").append(rateDescription.replace('*', '%')).append("' and"); 
				  }else {
	...
	/**
	 * To search rate class by rate class id, in this case oc id is mandatory
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchRateClass(Long ocId, Long rateClassId) {
		...
		int status = 0;
		try {
			  if (rateClassId != null && rateClassId != 0) {
		...
		try {
			discountRates = jdbcTemplate.queryForList(
					"SELECT discount_class.discount_class_id,discount_class.discount_class,discount_class.description,discount_class.is_package "
							+ " from discount_class where discount_class.oc_id = "
							+ ocId);
	...
	/**
	 * To search discount class details by oc id
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchDiscountClass(Long ocId) {
		List<Map<String, Object>> discountClassDetails = new ArrayList<>();
		StringBuilder query = new StringBuilder("SELECT oc.oc,discount_class.* FROM discount_class inner join oc on discount_class.oc_id = oc.oc_id WHERE ");
		int status = 0;
		try {
				appendOcId(ocId, query, discount);
				status++;
				if (status != 0) {
					query.append(" and is_package = 0)");
		...
		return discountClassDetails;
	}

	/**
	 * To search discount class by discount class, in this case oc id is mandatory
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchDiscountClass(Long ocId, String discountClass) {
		...
		int status = 0;
		try {
				if (isNotNullOrEmpty.test(discountClass)) {
					//search by * wild card & discount class
					  if(discountClass.contains("*")) {
						  query.append(" discount_class.discount_class like '").append(discountClass.replace('*', '%')).append("' and"); 
					  }else {
						  query.append(" discount_class.discount_class ='").append(discountClass).append("' and"); 
					  } 
					appendOcId(ocId, query, discount);
					status++;
				}
	...
	/**
	 * To search discount class by discount description, in this case oc id is mandatory
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchDiscountClass(long ocId, String discountDescription) {
		...
		int status = 0;
		try {
				 if (isNotNullOrEmpty.test(discountDescription)) {
					//search by * wild card & discount description
					 if(discountDescription.contains("*")) {
						  query.append(" discount_class.description like '").append(discountDescription.replace('*', '%')).append("' and"); 
					  }else {
						  query.append(" discount_class.description ='").append(discountDescription).append("' and"); 
					  } 
					appendOcId(ocId, query, discount);
					status++;
				}
	...
	/**
	 * To search discount class by discount class & discount description, in this case oc id is mandatory
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> searchDiscountClass(Long ocId, String discountClass, String discountDescription) {
		...
		int status = 0;
		try {
				 //search by * wild card & discountClass,discountDescription
				 if (isNotNullOrEmpty2.test(discountClass,discountDescription)) {
					 if(discountClass.contains("*")) {
						  query.append(" discount_class.discount_class like '").append(discountClass.replace('*', '%')).append("' and"); 
					  }else {
						  query.append(" discount_class.discount_class ='").append(discountClass).append("' and"); 
	...
	/**
	 * To search discount class by discount class id, in this case oc id is mandatory
	 */
	@Override
	public List<Map<String, Object>> searchDiscountClass(Long ocId, Long discountClassId) {
		...
		int status = 0;
		try {
				if (discountClassId != null && discountClassId != 0) {
	...
	/**
	 * To get promotion card effort details
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> getPromotionCardEffortDetails(Long promoCardId) {
		...
		try {
			  StringBuilder query = new StringBuilder("select promotion_card_id,promotion_card_from_effort,to_effort,rate_class.rate_class,discount_class.discount_class,source_code.source_code,");
			  query.append("rate_class.rate_class_id,discount_class.discount_class_id,source_code.source_code_id from promotion_card_from_effort ");
			  query.append("left join rate_class on promotion_card_from_effort.rate_class_id = rate_class.rate_class_id ");
			  query.append("left join discount_class on promotion_card_from_effort.discount_class_id = discount_class.discount_class_id ");
			  query.append("left join source_code on promotion_card_from_effort.source_code_id = source_code.source_code_id ");
			  query.append("where promotion_card_id=").append(promoCardId);
			  promoCardEffortList = jdbcTemplate.queryForList(query.toString());
		} catch (Exception e) {
			LOGGER.error(ERROR + e);
		}
	...
	/**
	 * To get promotion card offer details
	 * @author Chaman.Bharti
	 */
	@Override
	public List<Map<String, Object>> getPromotionCardOfferDetails(Long promoCardId, Long promoCardEffortId) {
		...
		try {
			  StringBuilder query = new StringBuilder("select promotion_card_id,promotion_card_from_effort,promotion_card_offer_seq,promotion_card_offer.description,order_code.order_code,order_code.order_code_id,order_code.active,");
			  query.append("subscription_def.subscription_def,subscription_def.subscription_def_id,product.product,product.product_id,pkg_def.pkg_def,pkg_def.pkg_def_id,");
			  query.append("rate_class.rate_class,rate_class.rate_class_id from promotion_card_offer ");
			  query.append("left join order_code on promotion_card_offer.order_code_id = order_code.order_code_id ");
			  query.append("left join subscription_def on promotion_card_offer.subscription_def_id = subscription_def.subscription_def_id and promotion_card_offer.order_code_id = subscription_def.order_code_id ");
			  query.append("left join pkg_def on  promotion_card_offer.pkg_def_id = pkg_def.pkg_def_id and promotion_card_offer.order_code_id = pkg_def.order_code_id ");
			  query.append("left join product on  promotion_card_offer.product_id = product.product_id and promotion_card_offer.order_code_id = product.order_code_id ");
			  query.append("left join rate_class on promotion_card_offer.rate_class_id = rate_class.rate_class_id ");
			  query.append("where promotion_card_id=").append(promoCardId).append(" and promotion_card_from_effort=").append(promoCardEffortId);
			  promoCardOfferList = jdbcTemplate.queryForList(query.toString());
		} catch (Exception e) {
			LOGGER.error(ERROR + e);
		}
	...
	public int savePromotionCardDetails(PromotionCard promotionCard) {
		...
		try {
			int maxPromotionCardId = getPromotionCardId() + 1;
			addSourceParams.put("promotion_card_id", maxPromotionCardId);
			...
			if(status !=0) {
				customerUtility.updateMruPromotionCardId(maxPromotionCardId);
			}
		} catch (Exception e) {
			LOGGER.error(ERROR + e);
		}
	...
	public int getPromotionCardId() {
		...
		try {
			String maxId = jdbcTemplate.queryForObject("select id from mru_promotion_card_id",
					String.class);
			if (maxId == null) {
				promotionCardId = 0;
			} else {
	...
	public int savePromotionCardEffortDetails(PromotionCardEffort promotionCardEffort) {
		...
		try {
			String maxPromotionCardFromEffort = jdbcTemplate.queryForObject(
					"select MAX(promotion_card_from_effort) from promotion_card_from_effort where promotion_card_id="+promotionCardEffort.getPromotionCardId(), String.class);
			...
			addSourceParams.put("mru_promotion_card_offer_seq",
					promotionCardEffort.getMruPromotionCardOfferSeq()!=null?promotionCardEffort.getMruPromotionCardOfferSeq(): null);
			addSourceParams.put("discount_class_id",
	...
	public int savePromotionCardOfferDetails(PromotionCardOffer promotionCardOffer) {
		...
		try {
			...
			if (maxPromotionCardOfferSeq == null) {
				...
				promotionCardOfferSeq = Integer.parseInt(maxPromotionCardOfferSeq) + 1;
				addSourceParams.put("promotion_card_offer_seq",promotionCardOfferSeq);
			}
			...
			addSourceParams.put("rate_class_id",
					promotionCardOffer.getRateClassId() != null ? promotionCardOffer.getRateClassId() : null);
			addSourceParams.put("order_code_id", promotionCardOffer.getOrderCodeId());
			addSourceParams.put("product_id",
					promotionCardOffer.getProductId() != null ? promotionCardOffer.getProductId() : null);
			addSourceParams.put("subscription_def_id",
					promotionCardOffer.getSubscriptionDefId() != null ? promotionCardOffer.getSubscriptionDefId() : null);
			...
			addSourceParams.put("pkg_def_id",
					promotionCardOffer.getPkgDefId() != null ? promotionCardOffer.getPkgDefId() : null);
			status = namedParameterJdbcTemplate.update(saveQuery.toString(), addSourceParams);
			if(status !=0) {
				updateMruPromotionCardOfferSeqColumn(promotionCardId, promotionCardEffortId,promotionCardOfferSeq);
			}
		} catch (Exception e) {
			LOGGER.error(ERROR + e);
		}
	...
	public void updateMruPromotionCardOfferSeqColumn(int promotionCardId, int promotionCardEffortId,int promotionCardOfferSeq) {
		...
		try {
			Map<String, Object> addOrderClassParams = new HashMap<>();
			addOrderClassParams.put("mru_promotion_card_offer_seq", promotionCardOfferSeq);
	...
	/**
	 * To get rate class effective details when click on Copy Details
	 * @author itee.gupta
	 */	
	@Override
	public Map<String, Object> getRateCardCopyDetails(Integer rate_class_id,Integer rate_class_effective_seq) 
	{
		...
		try 
		{
			if(rate_class_id>0)
			{
				if(rate_class_effective_seq>0)
				{
					int count=jdbcTemplate.queryForObject("select COUNT(*) from rate_class_effective WHERE rate_class_id = " + rate_class_id+"", Integer.class);
					int rateCount=jdbcTemplate.queryForObject("select COUNT(*) from ratecard where rate_class_id = " + rate_class_id+"", Integer.class);
					if(count>1 && rateCount>0 ) {					
					rateClassEffectiveList = jdbcTemplate.queryForList("SELECT * FROM rate_class_effective WHERE rate_class_id = " + rate_class_id+" AND mru_ratecard_seq is not null");
					}else {
					rateClassEffectiveList = jdbcTemplate.queryForList("SELECT * FROM rate_class_effective WHERE rate_class_id = " + rate_class_id+" ");
	...
	public int getProductId() {
		...
		return status;
				...
				public List<DropdownModel> getSourceCodeAttribute(String sourceFormat) {
					List<DropdownModel> sourceCodeAttribute = new ArrayList<>();
					try {
						List<Map<String, Object>> sourceAttribute = jdbcTemplate.queryForList(
								" select a.source_attribute,concat(a.source_attribute, ' : ' ,description) as attribute,'' as description,'' as width FROM source_format_segment s, source_attribute a WHERE s.source_format = '"+sourceFormat+"' and s.source_attribute = a.source_attribute ORDER BY source_format_segment_seq");
						for (Map<String, Object> sourceCode : sourceAttribute) {
							DropdownModel model = new DropdownModel();
							model.setKey(sourceCode.get("source_attribute").toString());
							model.setDisplay(sourceCode.get("attribute").toString());
							model.setDescription(sourceCode.get("description").toString());
							model.setExtra(sourceCode.get("width").toString());
							sourceCodeAttribute.add(model);
						}
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return sourceCodeAttribute;
				}
				...
				public List<DropdownModel> getAttributeValue(String sourceAttribute) {
					...
					try {
						List<Map<String, Object>> attribute = jdbcTemplate.queryForList(
								" SELECT source_attribute,source_attribute_value,description FROM source_attribute_value WHERE source_attribute = '"+sourceAttribute+"'");
						for (Map<String, Object> sourceCode : attribute) {
							DropdownModel model = new DropdownModel();
							model.setKey(sourceCode.get("source_attribute").toString());
							model.setDisplay(sourceCode.get("source_attribute_value").toString());
							model.setDescription(sourceCode.get("description").toString());
				...
				public List<Map<String, Object>> getState(String  sourceCodeId) {
					...
					try {
						StringBuilder query = new StringBuilder("select source_code_id,state,qty_mailed,cost_mailing from source_code_state where source_code_id = "+sourceCodeId);
						state = jdbcTemplate.queryForList(query.toString());
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return state;
				...
				public List<Map<String, Object>> getAttributeDetails(String  sourceCodeId) {
					List<Map<String, Object>> attributeDetails = new ArrayList<>();
					try {
						StringBuilder query = new StringBuilder("select a.source_attribute, concat(a.source_attribute, ' : ' ,a.description) as attribute,d.source_attribute_value,d.description from source_format_segment s left join source_attribute a on a.source_attribute= s.source_attribute left join source_code_attribute c on c.source_attribute = s.source_attribute left join source_attribute_value d on d.source_attribute_value = c.source_attribute_value and d.source_attribute = c.source_attribute WHERE source_code_id="+sourceCodeId+" and s.source_format = (select source_format from source_code where source_code_id="+sourceCodeId+")");
						attributeDetails = jdbcTemplate.queryForList(query.toString());
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
				...
				/**
				 * List of oc table details for source cod search
				 */
				@Override
				public List<Map<String, Object>> getOcDetails() {
					List<Map<String, Object>> ocDetails = new ArrayList<>();
					try {
						  StringBuilder query = new StringBuilder("select oc_id,oc,description,");
						  query.append("case when oc_type=0 then 'Parent Node' when oc_type=1 then 'Subscription OC' when oc_type=2 then 'Product OC' ");
						  query.append("when oc_type=3 then 'Basic Package OC' when oc_type=4 then 'Ad Hoc Package OC' when oc_type=5 then 'Pooled Package OC'");
						  query.append("when oc_type=6 then 'Quick Order OC' when oc_type=7 then 'Catalog OC' else 'Ship OC' end as oc_type from oc");
						  ocDetails = jdbcTemplate.queryForList(query.toString());
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return ocDetails;
				...
				private void appendSourceCodeQuery(StringBuilder query){
					query.append("SELECT oc,source_code_id,agency_customer_id,source_code.source_format,source_code,source_code.description,state_break,generated,breakeven,offer,list,");
					query.append("from_date,to_date,qty,cost,audit_qual_category,generic_agency,oc.oc_id,active,newsub_rate_class_id,new_renewal_card_id,newsub_discount_class_id,");
					query.append("audit_subscription_type_id,audit_qual_source_id,audit_name_title_id,audit_sales_channel_id,source_code.row_version,premium_order_code_id,");
					query.append("case when source_code.source_code_type=0 then 'Basic' else 'Dual' end as source_code_type,");
					query.append("mru_catalog_content_seq,currency,mru_generic_promotion_code_seq,is_ddp,shipping_price_list FROM source_code ");
				}

				/**
				 * To search source code by default
				 * @author Chaman.Bharti
				 */
				@Override
				public List<Map<String, Object>> searchSourceCode(long ocId, int active,int sourceCodeType) {
					...
					try {
							StringBuilder query = new StringBuilder();
							appendSourceCodeQuery(query);
							query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE  (active =").append(active!=0?active:1);
							appendOcId(ocId, query, searchSourceCode);
							//oc_type == 3 Basic Package OC, then source_code_type 1 or 2 else 0 or 2
							query.append(" and (source_code_type = ").append(getOcType(ocId)==3?1:0).append(" OR source_code_type = ").append(sourceCodeType!=0?sourceCodeType:2);
							query.append(")) ORDER BY source_code.oc_id,source_code.source_code");
							status++;
							if (status != 0) {
								sourceCodeDetails = jdbcTemplate.queryForList(query.toString());
							}
				...
				/**
				 * To search source code by sourceCode  & * wild card
				 * @author Chaman.Bharti
				 */
				@Override
				public List<Map<String, Object>> searchSourceCode(long ocId, int active,int sourceCodeType,String source_code) {
					List<Map<String, Object>> sourceCodeDetails = new ArrayList<>();
					int status = 0;
					try {
							StringBuilder query = new StringBuilder();
							appendSourceCodeQuery(query);
							if (isNotNullOrEmpty.test(source_code)) {
								//search by * wild card
								if(source_code.contains("*")){
									query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.source_code LIKE '").append(source_code.replace('*', '%')).append("' AND (active =").append(active!=0?active:1);
								}else {
									//search by source_code
									query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.source_code='").append(source_code).append("' AND (active =").append(active!=0?active:1);
								}
							}
							appendOcId(ocId, query, searchSourceCode);
							query.append(" and (source_code_type = ").append(getOcType(ocId)==3?1:0).append(" OR source_code_type = ").append(sourceCodeType!=0?sourceCodeType:2);
				...
				public List<Map<String, Object>> searchSourceCode(long ocId, int active, int sourceCodeType,String sourceCode, Long ocId2) {
					...
					try {
							StringBuilder query = new StringBuilder();
							appendSourceCodeQuery(query);
							if ((isNotNullOrEmpty.test(sourceCode) && ocId2 == null)) {
								//search by * wild card
								if(sourceCode.contains("*")) {
									query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.source_code LIKE '").append(sourceCode.replace('*', '%')).append("'");;
								}else {
									query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.source_code ='").append(sourceCode).append("'");;
								}
								status++;
							}
							if (isNotNullOrEmpty.test(sourceCode) && (status == 0 && ocId2 != null)) {
								//search by source code & order class
								if(sourceCode.contains("*")) {
									query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.source_code LIKE '").append(sourceCode.replace('*', '%')).append("'");;
								}else {
									query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.source_code ='").append(sourceCode).append("'");;
								}
								query.append(" AND source_code.oc_id=").append(ocId2).append(" AND (active =").append(active!=0?active:1);
								status++;
							}else if(status == 0 && ocId2 != null) {
								//search by order class
								query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.oc_id in(");
								query.append(ocId2).append(") AND (active =").append(active!=0?active:1);
							}
							appendOcId(ocId, query, searchSourceCode);
							query.append(" and (source_code_type = ").append(getOcType(ocId)==3?1:0).append(" OR source_code_type = ").append(sourceCodeType!=0?sourceCodeType:2);
							query.append(")) ORDER BY source_code.oc_id,source_code.source_code");
				...
				public List<Map<String, Object>> searchSourceCode(long ocId, int active, int sourceCodeType, Integer flag) {
					...
					try {
							...
							appendSourceCodeQuery(query);
							if (flag != null) {
								//search by system generated flag true & false
								query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.generated=").append(flag).append(" AND (active =").append(active!=0?active:1);
							}
							appendOcId(ocId, query, searchSourceCode);
							query.append(" and (source_code_type = ").append(getOcType(ocId)==3?1:0).append(" OR source_code_type = ").append(sourceCodeType!=0?sourceCodeType:2);
							query.append(")) ORDER BY source_code.oc_id,source_code.source_code");
				...
				public List<Map<String, Object>> getAuditDetails(String  sourceCodeId) {
					List<Map<String, Object>> auditDetails = null;
					try {
						if(sourceCodeId.equals("")) {
							StringBuilder query = new StringBuilder("select source_code_id,audit_qual_category,audit_subscription_type_id,audit_qual_source_id,audit_name_title_id,audit_sales_channel_id from source_code where source_code_id=null");
							auditDetails = jdbcTemplate.queryForList(query.toString());
						}else {
							StringBuilder query = new StringBuilder("select source_code_id,audit_qual_category,audit_subscription_type_id,audit_qual_source_id,audit_name_title_id,audit_sales_channel_id from source_code where source_code_id="+sourceCodeId);
							auditDetails = jdbcTemplate.queryForList(query.toString());
						}
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
				...
				public List<Map<String, Object>> searchSourceCode(long ocId, int active, int sourceCodeType,String sourceCode, Long ocId2, Integer flag) {
					...
					try {
							StringBuilder query = new StringBuilder();
							appendSourceCodeQuery(query);
							//search by source_code,order class & system generated
							if ((isNotNullOrEmpty.test(sourceCode) && status ==0) && (ocId2 != null && flag != null)) {
								query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
								if(sourceCode.contains("*")) {
									query.append("source_code.source_code like '").append(sourceCode.replace('*', '%'));
								}else {
									query.append("source_code.source_code='").append(sourceCode);
								}
								query.append("' AND source_code.oc_id=").append(ocId2).append(" AND source_code.generated=").append(flag).append(" AND (active =").append(active!=0?active:1);
								status++;
							}
							//search by source_code & system generated
							if ((isNotNullOrEmpty.test(sourceCode) && status ==0) && (flag !=null && ocId2 == null)) {
								if(sourceCode.contains("*")){
									query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.source_code LIKE '").append(sourceCode.replace('*', '%')).append("'");
								}else {
									query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.source_code= '").append(sourceCode).append("'");
								}
								query.append(" AND source_code.generated=").append(flag).append(" AND (active =").append(active!=0?active:1);
								status++;
							}
							//search by order class & system generated
							if ((sourceCode == null && status ==0) && (flag != null && ocId2 != null)) {
								query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.oc_id=").append(ocId2);
								query.append(" AND source_code.generated=").append(flag).append(" AND (active =").append(active!=0?active:1);
								status++;
							}
							
							//search by system generated
							if (flag != null && status ==0) {
								//search by system generated flag 1=true & 0=false
								query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE source_code.generated=").append(flag).append(" AND (active =").append(active!=0?active:1);
							}
							appendOcId(ocId, query, searchSourceCode);
							query.append(" and (source_code_type = ").append(getOcType(ocId)==3?1:0).append(" OR source_code_type = ").append(sourceCodeType!=0?sourceCodeType:2);
				...
				/**
				 * To search source code by offer from Offer\List tab
				 * @author Chaman.Bharti
				 */
				@Override
				public List<Map<String, Object>> searchSourceCodeFromOfferList(Long ocId, int active, int sourceCodeType,String offer) {
					...
					try {
							StringBuilder query = new StringBuilder();
							appendSourceCodeQuery(query);
							//search by offer, in this case ocId & offer is mandatory
							if (isNotNullOrEmpty.test(offer) && status ==0) {
								query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
								if(offer.contains("*")) {
									query.append("source_code.offer like '").append(offer.replace('*', '%')).append("'");
								}else {
									query.append("source_code.offer='").append(offer).append("'");;
								}
								status++;
							}
							query.append(" AND (active =").append(active!=0?active:1);
							appendOcId(ocId, query, searchSourceCode);
							query.append(" and (source_code_type = ").append(getOcType(ocId)==3?1:0).append(" OR source_code_type = ").append(sourceCodeType!=0?sourceCodeType:2);
							query.append(")) ORDER BY source_code.oc_id,source_code.source_code");
							if (status != 0) {
				...
				/**
				 * To search source code by list from Offer\List tab
				 * @author Chaman.Bharti
				 */
				@Override
				public List<Map<String, Object>> searchSourceCodeFromOfferList(long ocId, int active,int sourceCodeType, String list) {
					...
					try {
							StringBuilder query = new StringBuilder();
							appendSourceCodeQuery(query);
							//search by list, in this case ocId & list is mandatory
							if (isNotNullOrEmpty.test(list) && status ==0) {
								query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
								if(list.contains("*")) {
									query.append("source_code.list like '").append(list.replace('*', '%')).append("'");
								}else {
									query.append("source_code.list='").append(list).append("'");;
								}
								status++;
							}
							query.append(" AND (active =").append(active!=0?active:1);
				...
				/**
				 * To search source code by order class from Offer\List tab
				 * @author Chaman.Bharti
				 */
				@Override
				public List<Map<String, Object>> searchSourceCodeFromOfferList(long ocId, int active, int sourceCodeType,long ocId2) {
					...
					try {
							...
							query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
							query.append("source_code.oc_id=").append(ocId2);
							query.append(" AND (active =").append(active!=0?active:1);
							appendOcId(ocId, query, searchSourceCode);
							query.append(" and (source_code_type = ").append(getOcType(ocId)==3?1:0).append(" OR source_code_type = ").append(sourceCodeType!=0?sourceCodeType:2);
							query.append(")) ORDER BY source_code.oc_id,source_code.source_code");
				...
				/**
				 * To search source code by offer,list & order class in pairs from Offer\List tab
				 * @author Chaman.Bharti
				 */
				@Override
				public List<Map<String, Object>> searchSourceCodeFromOfferList(long ocId, int active, int sourceCodeType,String offer, String list, Long ocId2) {
					...
					try {
						StringBuilder query = new StringBuilder();
						appendSourceCodeQuery(query);
						//search by offer,list & order class
						if (isNotNullOrEmpty2.test(offer,list) && (ocId2 != null && status == 0)) {
							query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
							if(offer.contains("*")) {
								query.append("source_code.offer like '").append(offer.replace('*', '%')).append("'");
							}else {
							...
							if(list.contains("*")) {
								query.append(" AND source_code.list like '").append(list.replace('*', '%')).append("'");;
							}else {
								query.append(" AND source_code.list='").append(list).append("'");;
							}
							query.append(" AND source_code.oc_id=").append(ocId2).append(" AND (active =").append(active!=0?active:1);
							status++;
						}
						//search by offer & list
						if (isNotNullOrEmpty2.test(offer,list) && (ocId2 == null && status == 0)) {
							query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
							if(offer.contains("*")) {
								query.append("source_code.offer like '").append(offer.replace('*', '%')).append("'");
						...
						//search by offer & order class, in this case ocId2 is mandatory
						if (isNotNullOrEmpty.test(offer) && (list == null && status == 0)) {
							query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
							if(offer.contains("*")) {
								query.append("source_code.offer like '").append(offer.replace('*', '%')).append("'");
							}else {
						...
						//search by list & order class, in this case ocId,ocId2 is mandatory
						if (isNotNullOrEmpty.test(list) && (offer == null && status == 0)) {
							query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
							if(list.contains("*")) {
								query.append("source_code.list like '").append(list.replace('*', '%')).append("'");
							}else {
				...
				@Override
				public List<Map<String, Object>> getPromotionCardDetails(int promoCardId) {
					List<Map<String, Object>> promoCardList = new ArrayList<>();
					try {
						  promoCardList = jdbcTemplate.queryForList("select promotion_card_id,oc_id,promotion_card,description from promotion_card where promotion_card_id="+promoCardId);
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return promoCardList;
				...
				public int getOcType(long ocId) {
					Integer ocType = null;
					try {
						  ocType = jdbcTemplate.queryForObject("select oc_type from oc where oc_id="+ocId,Integer.class);
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return  ocType!=null?ocType:0;
				}
				...
				public List<Map<String, Object>> addressLookUp() {
					List<Map<String, Object>> promoCardList = new ArrayList<>();
					try {
						  promoCardList = jdbcTemplate.queryForList("select * from view_address_lookup");
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return promoCardList;
				...
				public List<Map<String, Object>> addressLookUp(Long ocId, int active, int sourceCodeType,Integer addressId, String description) {
					int status = 0;
					List<Map<String, Object>> addressLookUpDetails = new ArrayList<>();
					try {
						StringBuilder query = new StringBuilder();
						appendSourceCodeQuery(query);
						//search by address id & description
						if (isNotNullOrEmpty.test(description) && (ocId != null && addressId != null)) {
							
						}
						//search by description
						if (isNotNullOrEmpty.test(description) && (addressId == null && status == 0)) {
							query.append("inner join oc on source_code.oc_id = oc.oc_id WHERE ");
							if(description.contains("*")) {
								query.append("source_code.description like '").append(description.replace('*', '%')).append("'");
							}else {
								query.append("source_code.description='").append(description).append("'");;
							}
							query.append(" AND (active =").append(active!=0?active:1);
							status++;
						}
						//search by id, in this case ocId is mandatory
						if (addressId!=null && (description == null && status == 0)) {
						...
						query.append(")) ORDER BY source_code.oc_id,source_code.source_code");
						if (status != 0) {
							addressLookUpDetails = jdbcTemplate.queryForList(query.toString());
						}
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return addressLookUpDetails;
				}
				...
				public int deletePromotionOffer(int promotionCardId, int promotionCardFromEffort,int promotionCardOfferSeq) {
					int Status = 0;
					try {
							StringBuilder deletePromotionCard = new StringBuilder("delete from promotion_card_offer where promotion_card_id=").append(promotionCardId);
							deletePromotionCard.append(" and promotion_card_from_effort=").append(promotionCardFromEffort);
							deletePromotionCard.append(" and promotion_card_offer_seq=").append(promotionCardOfferSeq);
							Status = jdbcTemplate.update(deletePromotionCard.toString());
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return Status;
				...
				public String deletePromotionCardFromEffort(int promotionCardId, int promotionCardFromEffort) {
					...
					String message = null;
					try {
							status = jdbcTemplate.queryForObject("select count(*) as count from promotion_card_offer where promotion_card_id="+promotionCardId+" and promotion_card_from_effort="+promotionCardFromEffort, Integer.class);
							//if count is 0 means promotionCardId & promotionCardFromEffort are not present in promotion_card_offer then it can be deleted.
							if(status == 0) {
								StringBuilder deletePromotionCardFromEffort = new StringBuilder("delete from promotion_card_from_effort where promotion_card_id=").append(promotionCardId);
								deletePromotionCardFromEffort.append(" and promotion_card_from_effort=").append(promotionCardFromEffort);
								status = jdbcTemplate.update(deletePromotionCardFromEffort.toString());
								message = "deleted";
							}else {
								StringBuilder msg = new StringBuilder("Error in trigger td_promotion_card_from_effort 30006: ");
								msg.append("Children still exist in 'promotion_card_offer'. Cannot delete ");
								msg.append("parent 'promotion_card_from_effort' ");
								msg.append("The transaction ended in the trigger. The batch has been aborted.");
				...
				public String deletePromotionCard(int promotionCardId) {
					...
					String message = null;
					try {
							status = jdbcTemplate.queryForObject("select count(*) as count from promotion_card_from_effort where promotion_card_id="+promotionCardId, Integer.class);
							//if count is 0 means promotionCardId is not present in promotion_card_from_effort then it can be deleted.
							if(status == 0) {
								StringBuilder deletePromotionCard = new StringBuilder("delete from promotion_card where promotion_card_id=").append(promotionCardId);
								status = jdbcTemplate.update(deletePromotionCard.toString());
								message = "deleted";
							}else {
								StringBuilder msg = new StringBuilder("Error in trigger td_promotion_card 30006: Children still exist");
								msg.append("in 'promotion_card_from_effort'. Cannot delete parent 'promotion_card'.");
								msg.append("The transaction ended in the trigger. The batch has been aborted.");
								message = msg.toString();
							}
					} catch (Exception e) {
				...
				private void appendSubscriptionDefQuery(StringBuilder query){
					query.append("SELECT subscription_def,subscription_def.term_id,media,edition,subscription_category.description as category,subscription_category.subscription_category_id,");
					query.append("subscription_def.order_code_id,oc_id,subscription_def.renewal_card_id,subscription_def.pub_rotation_id,subscription_def.rate_class_id,subscription_def_id,");
					query.append("subscription_def.allow_on_internet,subscription_def.tag_line,subscription_def.issn,subscription_def.row_version,subscription_def.expire_gap,subscription_def.logical_start,");
					query.append("subscription_def.audit_n_grace_issues_allowed,subscription_def.auxiliary_1,subscription_def.auxiliary_2,subscription_def.forced_expire_month,subscription_def.extfree_qty,");
					query.append("subscription_def.upgrade_downgrade_value,subscription_def.cancel_policy_id,subscription_def.inactive,subscription_def,subscription_def.premium_order_code_id ");
					query.append("FROM subscription_def ");
					query.append("left join subscription_category on subscription_def.subscription_category_id = subscription_category.subscription_category_id WHERE ");
				}
				/**
				 * To search subscription definition
				 * @author Chaman.Bharti
				 */
				@Override
				public List<Map<String, Object>> searchSubscriptionDef(int orderCodeId, Integer inactive,String description, Integer termId, String media, Integer categoryId,String action) {
					...
					try {
						StringBuilder query = new StringBuilder();
						appendSubscriptionDefQuery(query);
						switch (action) {
						
						//search by default case when user click grid of promotion offer
						case "default":
							status++;
						break;
						//search by description,termId,media & subCategoryId
						case "all":
							if (isNotNullOrEmpty2.test(description,media) && isNotNull.test(termId,categoryId)) {
								if(description.contains("*")) {
									query.append("subscription_def.description like '").append(description.replace('*', '%')).append("'");
								}else {
									query.append("subscription_def.description ='").append(description).append("'");;
								}
								query.append(" AND subscription_def.term_id=").append(termId);
								if(media.contains("*")) {
									query.append(" AND subscription_def.media like '").append(media.replace('*', '%')).append("'");;
								}else {
									query.append(" AND subscription_def.media='").append(media).append("'");;
								}
								query.append(" AND subscription_def.subscription_category_id=").append(categoryId).append(" AND ");
								status++;
							}
						break;
						//search by description,media & termId
						case "descriptionTermMedia":
							if (isNotNullOrEmpty2.test(description,media) && (termId != null)) {
								if(description.contains("*")) {
									query.append("subscription_def.description like '").append(description.replace('*', '%')).append("'");
								}else {
									query.append("subscription_def.description ='").append(description).append("'");;
								...
								query.append(" AND subscription_def.term_id=").append(termId);
								if(media.contains("*")) {
									query.append(" AND subscription_def.media like '").append(media.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append(" AND subscription_def.media='").append(media).append("'").append(" AND ");
								}
							status++;
						  }
						break;
							//search by description & termId
						case "descriptionTerm":
							if (isNotNullOrEmpty.test(description) && (termId != null)) {
								if(description.contains("*")) {
									query.append("subscription_def.description like '").append(description.replace('*', '%')).append("'");
								}else {
									query.append("subscription_def.description ='").append(description).append("'");;
								}
								query.append(" AND subscription_def.term_id=").append(termId).append(" AND ");;
							status++;
						  }
						break;
						//search by description & media
						case "descriptionMedia":
							if (isNotNullOrEmpty2.test(description,media)) {
								if(description.contains("*")) {
									query.append("subscription_def.description like '").append(description.replace('*', '%')).append("'");
								}else {
									query.append("subscription_def.description ='").append(description).append("'");;
						...
						break;
						//search by description & categoryId
						case "descriptionCategory":
							if (isNotNullOrEmpty.test(description) && categoryId != null) {
								if(description.contains("*")) {
									query.append("subscription_def.description like '").append(description.replace('*', '%')).append("'");
								}else {
									query.append("subscription_def.description ='").append(description).append("'");;
						...
						break;
						case "description":
							if (isNotNullOrEmpty.test(description) && orderCodeId != 0) {
								if(description.contains("*")) {
									query.append("subscription_def.description like '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("subscription_def.description ='").append(description).append("'").append(" AND ");
								}
							status++;
						  }
						break;
						//search by termId,media & category
						case "termMediaCategory":
							if (isNotNullOrEmpty.test(media) && isNotNull.test(termId,categoryId)) {
								query.append("subscription_def.term_id=").append(termId);
								if(media.contains("*")) {
									query.append(" AND subscription_def.media like '").append(media.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append(" AND subscription_def.media='").append(media).append("'").append(" AND ");
								}
								query.append("subscription_def.subscription_category_id=").append(categoryId).append(" AND ");
							status++;
						  }
						break;
						//search by termId,media
						case "termMedia":
							if (isNotNullOrEmpty.test(media) && termId != null) {
								query.append("subscription_def.term_id=").append(termId);
								if(media.contains("*")) {
									query.append(" AND subscription_def.media like '").append(media.replace('*', '%')).append("'").append(" AND ");
								}else {
						...
						break;
						case "termCategory":
							if (isNotNull.test(termId,categoryId)) {
								query.append("subscription_def.term_id=").append(termId);
								query.append(" AND subscription_def.subscription_category_id=").append(categoryId).append(" AND ");
							status++;
						  }
						...
						case "term":
							if (termId != null) {
								query.append("subscription_def.term_id=").append(termId);
								query.append(categoryId).append(" AND ");
							status++;
						  }
						break;
						case "mediaCategory":
							if (isNotNullOrEmpty.test(media) && categoryId != null) {
								if(media.contains("*")) {
									query.append(" AND subscription_def.media like '").append(media.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append(" AND subscription_def.media='").append(media).append("'").append(" AND ");
						...
						break;
						case "media":
							if (isNotNullOrEmpty.test(media)) {
								if(media.contains("*")) {
									query.append("subscription_def.media like '").append(media.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("subscription_def.media='").append(media).append("'").append(" AND ");
								}
							status++;
						  }
						break;
						case "categoryMediaDescription":
							if (isNotNullOrEmpty2.test(media,description) && categoryId != null) {
								query.append("subscription_def.subscription_category_id=").append(categoryId).append(" AND ");
								if(media.contains("*")) {
									query.append("subscription_def.media like '").append(media.replace('*', '%')).append("'").append(" AND ");
								}else {
						...
						query.append("(inactive =").append(inactive!=null?inactive:0).append(" and order_code_id =").append(orderCodeId);
						query.append(") ORDER BY subscription_def.subscription_def_id");
						if (status != 0) {
							subscriptionDetails = jdbcTemplate.queryForList(query.toString());
						}
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
				...
				public List<Map<String, Object>> searchPkgDef(int orderCodeId, Integer active, Integer pkgDefId,String pkgDef, String description,String action) {
					int status = 0;
					List<Map<String, Object>> pkgDefDetails = new ArrayList<>();
					try {
						StringBuilder query = new StringBuilder("SELECT pkg_def_id,pkg_def,description,n_calendar_units,case when calendar_unit=0 then 'Day(s)' ");
						query.append("when calendar_unit=1 then 'Week(s)' when calendar_unit=2 then 'Month(s)' ");
						query.append("when calendar_unit=3 then 'Year(s)' end as calendar_unit,");
						query.append("pkg_price,pkg_price_method,renewal_card_id,discount_class_id,qty_discount_schedule,active,");
						query.append("subscriber_site_allowance_type,order_code_type,pkg_price_method,order_code_id,rate_class_id,");
						query.append("revenue_percentage_option,oc_id,row_version,pkg_contents_together,multiline_discount_schedule,auxiliary_1,auxiliary_2,");
						query.append("pkg_start_match_order_date,premium_order_code_id ");
						query.append("FROM view_pkg_def WHERE ");
						switch (action) {
						...
						case "default":
							status++;
						break;
						//search by pkgDef & description
						case "pkgDefDescription":
							if (isNotNullOrEmpty2.test(pkgDef,description) && active != null) {
								if(pkgDef.contains("*")) {
									query.append("view_pkg_def.pkg_def LIKE '").append(pkgDef.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("view_pkg_def.pkg_def ='").append(pkgDef).append("'").append(" AND ");
								}
								if(description.contains("*")) {
									query.append("view_pkg_def.description LIKE '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("view_pkg_def.description ='").append(description).append("'").append(" AND ");
								}
								status++;
							}
						break;
						//search by pkgDef
						case "pkgDef":
							if (isNotNullOrEmpty.test(pkgDef) && active != null) {
								if(pkgDef.contains("*")) {
									query.append("view_pkg_def.pkg_def LIKE '").append(pkgDef.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("view_pkg_def.pkg_def ='").append(pkgDef).append("'").append(" AND ");
						...
						//search by description
						case "description":
							if (isNotNullOrEmpty.test(description) && active != null) {
								if(description.contains("*")) {
									query.append("view_pkg_def.description LIKE '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("view_pkg_def.description ='").append(description).append("'").append(" AND ");
						...
						//search by pkdDefId
						case "pkgDefId":
							if (isNotNull.test(pkgDefId,active)) {
								query.append("view_pkg_def.pkg_def_id = ").append(pkgDefId).append(" AND ");
								status++;
							}
						break;
						
						}
						query.append(" (order_code_id = ").append(orderCodeId).append(" and active = ").append(active!=null?active:1).append(")");
						if (status != 0) {
							pkgDefDetails = jdbcTemplate.queryForList(query.toString());
						}
								
					} catch (Exception e) {
						LOGGER.error(ERROR + e);
					}
					return pkgDefDetails;
				}
				...
				public List<Map<String, Object>> searchProduct(int orderCodeId, String description, String style,String size, String color, String action) {
					...
					List<Map<String, Object>> productDetails = new ArrayList<>();
					try {
						  StringBuilder query = new StringBuilder("SELECT product.* FROM product WHERE ");
						  switch (action) {
						//search by default case when user click grid of promotion offer
						case "default":
							status++;
						...
						case "all":
							if (isNotNullOrEmpty2.test(description,style) && isNotNullOrEmpty2.test(size,color)) {
								if(description.contains("*")) {
									query.append("product.description like '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.description ='").append(description).append("'").append(" AND ");
								}
								if(style.contains("*")) {
									query.append("product.product_style like '").append(style.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_style ='").append(style).append("'").append(" AND ");
								}
								if(size.contains("*")) {
									query.append("product.product_size like '").append(size.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_size ='").append(size).append("'").append(" AND ");
								}
								if(color.contains("*")) {
									query.append("product.product_color like '").append(color.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_color ='").append(color).append("'").append(" AND ");
								}
								status++;
							}
						break;
						//search by description
						case "description":
							if (isNotNullOrEmpty.test(description)) {
								if(description.contains("*")) {
									query.append("product.description like '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.description ='").append(description).append("'").append(" AND ");
						...
						break;
						//search by description,style,size
						case "descriptionStyleSize":
							if (isNotNullOrEmpty2.test(description,style) && isNotNullOrEmpty.test(size)) {
								if(description.contains("*")) {
									query.append("product.description like '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.description ='").append(description).append("'").append(" AND ");
						...
						break;
						//search by description,size & color
						case "descriptionSizeColor":
							if (isNotNullOrEmpty2.test(description,size) && isNotNullOrEmpty.test(color)) {
								if(description.contains("*")) {
									query.append("product.description like '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.description ='").append(description).append("'").append(" AND ");
						...
						break;
						//search by description,style
						case "descriptionStyle":
							if (isNotNullOrEmpty2.test(description,style)) {
								if(description.contains("*")) {
									query.append("product.description like '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.description ='").append(description).append("'").append(" AND ");
						...
						case "descriptionSize":
							if (isNotNullOrEmpty2.test(description,size)) {
								if(description.contains("*")) {
									query.append("product.description like '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.description ='").append(description).append("'").append(" AND ");
						...
						break;
						//search by description & color
						case "descriptionColor":
							if (isNotNullOrEmpty2.test(description,color)) {
								if(description.contains("*")) {
									query.append("product.description like '").append(description.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.description ='").append(description).append("'").append(" AND ");
						...
						//search by style,size & color
						case "styleSizeColor":
							if (isNotNullOrEmpty2.test(style,size) && isNotNullOrEmpty.test(color)) {
								if(style.contains("*")) {
									query.append("product.product_style like '").append(style.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_style ='").append(style).append("'").append(" AND ");
						...
						break;
						//search by style,size
						case "styleSize":
							if (isNotNullOrEmpty2.test(style,size)) {
								if(style.contains("*")) {
									query.append("product.product_style like '").append(style.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_style ='").append(style).append("'").append(" AND ");
						...
						//search by style
						case "style":
							if (isNotNullOrEmpty.test(style)) {
								if(style.contains("*")) {
									query.append("product.product_style like '").append(style.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_style ='").append(style).append("'").append(" AND ");
						...
						case "styleColor":
							if (isNotNullOrEmpty2.test(style,color)) {
								if(style.contains("*")) {
									query.append("product.product_style like '").append(style.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_style ='").append(style).append("'").append(" AND ");
						...
						case "sizeColor":
							if (isNotNullOrEmpty2.test(size,color)) {
								if(size.contains("*")) {
									query.append("product.product_size like '").append(size.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_size ='").append(size).append("'").append(" AND ");
						...
						//search by color
						case "color":
							if (isNotNullOrEmpty.test(color)) {
								if(color.contains("*")) {
									query.append("product.product_color like '").append(color.replace('*', '%')).append("'").append(" AND ");
								}else {
									query.append("product.product_color ='").append(color).append("'").append(" AND ");
						...
						query.append("(order_code_id =").append(orderCodeId).append(") ORDER BY product.product_id");
						if (status != 0) {
							productDetails = jdbcTemplate.queryForList(query.toString());
						}
					} catch (Exception e) {
				...
				public List<Map<String, Object>> getAuditCategory(int ocId) {
					...
					try {
						StringBuilder query2 = new StringBuilder("select oc_id,qp as qualifiedPaid,qf as qualifiedFree,nqp as nonQualifiedPaid,nqf as nonQualifiedfree from pub where oc_id="+ocId);
						audit = jdbcTemplate.queryForList(query2.toString());
